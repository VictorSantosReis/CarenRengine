...
//Métodos da interface proprietária(ICarenD3D11DeviceContext4)
...
CarenResult Shared_Direct3D11DeviceContext::Signal(ID3D11DeviceContext4* Param_MyPointerWork, ICarenD3D11Fence^ Param_Fence, UInt64 Param_Valor)
{
	...
	Resultado = Param_Fence->RecuperarPonteiro((LPVOID*)&pFence);

	//Verifica se não houve erro.
	if (Resultado.StatusCode != ResultCode::SS_OK)
	{
		//O ponteiro para a interface não é valido..
	...
	Hr = Param_MyPointerWork->Signal(pFence, Param_Valor);
...
CarenResult Shared_Direct3D11DeviceContext::Wait(ID3D11DeviceContext4* Param_MyPointerWork, ICarenD3D11Fence^ Param_Fence, UInt64 Param_Valor)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->Wait(pFence, Param_Valor);
...
CarenResult Shared_Direct3D11DeviceContext::Flush1(ID3D11DeviceContext3* Param_MyPointerWork, CA_D3D11_CONTEXT_TYPE Param_TypeContexto, ICarenEvent^ Param_Handle)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Variaveis a serem utilizadas.
	HANDLE pHandle = NULL;
	D3D11_CONTEXT_TYPE TipoContexto = static_cast<D3D11_CONTEXT_TYPE>(Param_TypeContexto);

	//Verifica se forneceu uma handle para evento
	if (ObjetoGerenciadoValido(Param_Handle))
	{
		//Como o usuário informou uma handle, mesmo que opcional, se essa não for valida o método retornar ER_PONTEIRO.

		//Recupera a handle para o evento.
		Resultado = Param_Handle->RecuperarEvento(&pHandle);

		//Verifica se não houve erro
		if (Resultado.StatusCode != ResultCode::SS_OK)
		{
			//A handle para o evento é invalida...
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->Flush1(TipoContexto, pHandle);
...
CarenResult Shared_Direct3D11DeviceContext::GetHardwareProtectionState(ID3D11DeviceContext3* Param_MyPointerWork, [Out] bool% Param_Out_ProtecaoHardwareHabilitada)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);

	//Variaveis a serem utilizadas.
	BOOL StatusProtecao = FALSE;

	//Chama o método para realizar a operação.
	Param_MyPointerWork->GetHardwareProtectionState(&StatusProtecao);

	//Define o resutlado no parametro de saida.
	Param_Out_ProtecaoHardwareHabilitada = static_cast<bool>(StatusProtecao);
...
CarenResult Shared_Direct3D11DeviceContext::SetHardwareProtectionState(ID3D11DeviceContext3* Param_MyPointerWork, bool Param_EstadoProtecao)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);

	//Chama o método para realizar a operação.
	Param_MyPointerWork->SetHardwareProtectionState(Param_EstadoProtecao ? TRUE : FALSE);
...
CarenResult Shared_Direct3D11DeviceContext::BeginEventInt(
	...
	Int32 Param_Data)
{
	...
	//Verifica se uma string foi fornecida e converte para nativa.
	if (!String::IsNullOrEmpty(Param_Label))
	{
		//Converte a string
		pLabel = Util.ConverterStringToWCHAR(Param_Label);
	}

	//Chama o método para realizar a operação.
	Param_MyPointerWork->BeginEventInt(pLabel, Param_Data);

	//Define sucesso na operação
	Resultado.AdicionarCodigo(ResultCode::SS_OK, true);

	//Libera a memoria para a string se valida
	if (ObjetoValido(pLabel))
	{
...
CarenResult Shared_Direct3D11DeviceContext::CopyTileMappings(
	ID3D11DeviceContext2* Param_MyPointerWork,
	...
	CA_D3D11_TILE_MAPPING_FLAGS Param_Flags)
{
	...
	D3D11_TILED_RESOURCE_COORDINATE* pCoordenadasDestino = NULL;
	D3D11_TILED_RESOURCE_COORDINATE* pCoordenadasOrigem = NULL;
	D3D11_TILE_REGION_SIZE* pSizeRegiao = NULL;
	UINT FlagsMetodo = static_cast<UINT>(Param_Flags);

	//Obtém os ponteiros para os recursos

	//Obtém o ponteiro para o recurso de DESTINO.
	Resultado = Param_DestinoRecursoTiled->RecuperarPonteiro((LPVOID*)&pRecursoDestino);
	...
	Resultado = Param_OrigemRecursoTiled->RecuperarPonteiro((LPVOID*)&pRecursoOrigem);
	...
	pCoordenadasDestino = Util.ConverterD3D11_TILED_RESOURCE_COORDINATEManaged_ToUnManaged(Param_DestinoCoordenadasInicio);
	pCoordenadasOrigem = Util.ConverterD3D11_TILED_RESOURCE_COORDINATEManaged_ToUnManaged(Param_OrigemCoordenadasInicio);
	pSizeRegiao = Util.ConverterD3D11_TILE_REGION_SIZEManaged_ToUnManaged(Param_TamanhoRegiaoTiled);

	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CopyTileMappings(pRecursoDestino, pCoordenadasDestino, pRecursoOrigem, pCoordenadasOrigem, pSizeRegiao, FlagsMetodo);
...
Done:;
	...
	DeletarEstruturaSafe(&pCoordenadasDestino);
	DeletarEstruturaSafe(&pCoordenadasOrigem);
	DeletarEstruturaSafe(&pSizeRegiao);
...
CarenResult Shared_Direct3D11DeviceContext::CopyTiles(
	ID3D11DeviceContext2* Param_MyPointerWork,
	...
	CA_D3D11_TILE_COPY_FLAG Param_CopyFlags)
{
	...
	D3D11_TILED_RESOURCE_COORDINATE* pCoordenadasInicio = NULL;
	D3D11_TILE_REGION_SIZE* pRegiaoSize = NULL;
	UINT CopyFlags = static_cast<UINT>(Param_CopyFlags);


	//Obtém o ponteiro para o recurso.
	Resultado = Param_RecursoTiled->RecuperarPonteiro((LPVOID*)&pRecurso);
	...
	//Converte as estruturas.
	pCoordenadasInicio = Util.ConverterD3D11_TILED_RESOURCE_COORDINATEManaged_ToUnManaged(Param_CoordenadasInicio);
	pRegiaoSize = Util.ConverterD3D11_TILE_REGION_SIZEManaged_ToUnManaged(Param_TamanhoRegiaoTile);

	//Chama o método para realizar a operação.
	Param_MyPointerWork->CopyTiles(pRecurso, pCoordenadasInicio, pRegiaoSize, pBuffer, Param_BufferStartOffsetInBytes, CopyFlags);
...
Done:;
	//Libera a memória para as estruturas.
	DeletarEstruturaSafe(&pCoordenadasInicio);
	DeletarEstruturaSafe(&pRegiaoSize);
...
/// </summary>
CarenResult Shared_Direct3D11DeviceContext::EndEvent(ID3D11DeviceContext2* Param_MyPointerWork)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);

	//Chama o método para realizar a operação.
	Param_MyPointerWork->EndEvent();
...
CarenResult Shared_Direct3D11DeviceContext::IsAnnotationEnabled(ID3D11DeviceContext2* Param_MyPointerWork, [Out] bool% Param_Out_Estado)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);

	//Variaveis a serem utilizadas.
	BOOL StatusAnnotation = FALSE;

	//Chama o método para realizar a operação.
	StatusAnnotation = Param_MyPointerWork->IsAnnotationEnabled();

	//Define no parametro de saida.
	Param_Out_Estado = static_cast<bool>(StatusAnnotation);
...
CarenResult Shared_Direct3D11DeviceContext::ResizeTilePool(
	ID3D11DeviceContext2* Param_MyPointerWork,
	...
	UInt64 Param_NovoSizeBytes)
{
	...
	//Variaveis a serem utilizadas.
	ID3D11Buffer* pBufferTile = NULL;

	//Obtém o ponteiro para o Buffer.
	Resultado = Param_TilePool->RecuperarPonteiro((LPVOID*)&pBufferTile);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->ResizeTilePool(pBufferTile, Param_NovoSizeBytes);
...
/// <param name="Param_Label">Uma sequência opcional que será registrada na ETW quando o registro ETW estiver ativo. Se '#d' aparecer na sequência, ele será substituído pelo valor do parâmetro Param_Data semelhante à forma como o printf funciona.</param>
/// <param name="Param_Data">Um valor de dados assinado que será registrado na ETW quando o registro ETW estiver ativo.</param>
CarenResult Shared_Direct3D11DeviceContext::SetMarkerInt(
	ID3D11DeviceContext2* Param_MyPointerWork,
	String^ Param_Label,
	Int32 Param_Data)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->SetMarkerInt(pLabel, Param_Data);
...
CarenResult Shared_Direct3D11DeviceContext::TiledResourceBarrier(
	ID3D11DeviceContext2* Param_MyPointerWork,
	...
	ICarenD3D11DeviceChild^ Param_TiledResourceOrViewAccessAfterBarrier)
{
	...
	//Variaveis a serem utilizadas.
	ID3D11DeviceChild* pDispositivoFilho_Before = NULL;
	ID3D11DeviceChild* pDispositivoFilho_After = NULL;

	//Recupera o ponteiro para as interfaces
	Resultado = Param_TiledResourceOrViewAccessBeforeBarrier->RecuperarPonteiro((LPVOID*)&pDispositivoFilho_Before);

	//Verifica o resultado
	if (Resultado.StatusCode != ResultCode::SS_OK)
	{
		//O ponteiro é invalido
	...
	//Recupera o ponteiro para as interfaces
	Resultado = Param_TiledResourceOrViewAccessAfterBarrier->RecuperarPonteiro((LPVOID*)&pDispositivoFilho_After);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->TiledResourceBarrier(pDispositivoFilho_Before, pDispositivoFilho_After);
...
CarenResult Shared_Direct3D11DeviceContext::UpdateTileMappings(
	ID3D11DeviceContext2* Param_MyPointerWork,
	ICarenD3D11Resource^ Param_RecursoTiled,
	...
	CA_D3D11_TILE_MAPPING_FLAGS Param_Flags)
{
	...
	Utilidades Util;
	ID3D11Resource* pRecurso = NULL;
	ID3D11Buffer* pBufferTilePool = NULL;
	D3D11_TILED_RESOURCE_COORDINATE* pMatriz_CoordenadasIniciais = CriarMatrizEstruturas<D3D11_TILED_RESOURCE_COORDINATE>(Param_QuantidadeReigoesRecursoTiled);
	D3D11_TILE_REGION_SIZE* pMatriz_SizeRegioes = CriarMatrizEstruturas<D3D11_TILE_REGION_SIZE>(Param_QuantidadeReigoesRecursoTiled);;
	UINT* pMatriz_FlagsRange = CriarMatrizUnidimensional<UINT>(Param_NumeroRanges);
	UINT32* pMatriz_OffsetStart = CriarMatrizUnidimensional<UINT32>(Param_MatrizOffsetStartTiledPool->Length);
	UINT32* pMatriz_QuantidadeRanges = CriarMatrizUnidimensional<UINT32>(Param_NumeroRanges);
	UINT32 FlagsMetodo = static_cast<UINT32>(Param_Flags);
	D3D11_TILED_RESOURCE_COORDINATE* pGetMatrizCord = NULL;
	D3D11_TILE_REGION_SIZE* pGetMatrizRegiaoSize = NULL;
	...
	//Recupera o ponteiro para a interface de Buffer.
	Resultado = Param_TilePool->RecuperarPonteiro((LPVOID*)&pBufferTilePool);
	...
	//Faz um for para converter as matizes de estruturas gerenciadas para a nativa.

	//Converte a matriz com as COORDENADAS.
	for (UINT32 i = 0; i < Param_QuantidadeReigoesRecursoTiled; i++)
	{
		//Converte a estrutura.
		pGetMatrizCord = Util.ConverterD3D11_TILED_RESOURCE_COORDINATEManaged_ToUnManaged(Param_MatrizCoordernadasInicioRecursoTiled[i]);

		//Desreferencia o ponteiro e obtém os dados.
		pMatriz_CoordenadasIniciais[i] = *pGetMatrizCord;

		//Libera a memória para a matriz.
		DeletarEstruturaSafe(&pGetMatrizCord);
	}

	//Converte a matriz com os SIZE REGIONS.
	for (UINT32 i = 0; i < Param_QuantidadeReigoesRecursoTiled; i++)
	{
		//Converte a estrutura.
		pGetMatrizRegiaoSize = Util.ConverterD3D11_TILE_REGION_SIZEManaged_ToUnManaged(Param_MatrizSizeReigoesRecursoTiled[i]);

		//Desreferencia o ponteiro e obtém os dados.
		pMatriz_SizeRegioes[i] = *pGetMatrizRegiaoSize;

		//Libera a memória para a matriz.
		DeletarEstruturaSafe(&pGetMatrizRegiaoSize);
	}

	//Copia os dados das demais matrizes.
	Util.CopiarItensTo_ArrayNativo(&pMatriz_FlagsRange, Param_MatrizFlagsRange, Param_NumeroRanges);
	Util.CopiarItensTo_ArrayNativo(&pMatriz_OffsetStart, Param_MatrizOffsetStartTiledPool, Param_MatrizOffsetStartTiledPool->Length);
	Util.CopiarItensTo_ArrayNativo(&pMatriz_QuantidadeRanges, Param_MatrizQuantidadeRangesTile, Param_NumeroRanges);

	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->UpdateTileMappings(
		pRecurso,
		Param_QuantidadeReigoesRecursoTiled,
		pMatriz_CoordenadasIniciais,
		pMatriz_SizeRegioes,
		pBufferTilePool,
		Param_NumeroRanges,
		pMatriz_FlagsRange,
		pMatriz_OffsetStart,
		pMatriz_QuantidadeRanges,
		FlagsMetodo);
...
Done:;
	...
	DeletarEstruturaSafe(&pMatriz_CoordenadasIniciais);
	DeletarEstruturaSafe(&pMatriz_SizeRegioes);
	DeletarMatrizUnidimensionalSafe(&pMatriz_FlagsRange);
	DeletarMatrizUnidimensionalSafe(&pMatriz_OffsetStart);
	DeletarMatrizUnidimensionalSafe(&pMatriz_QuantidadeRanges);
...
CarenResult Shared_Direct3D11DeviceContext::UpdateTiles(
	ID3D11DeviceContext2* Param_MyPointerWork,
	...
	CA_D3D11_TILE_COPY_FLAG Param_CopyFlags)
{
	...
	D3D11_TILED_RESOURCE_COORDINATE* pCoordnadasInicio = NULL;
	D3D11_TILE_REGION_SIZE* pRegiaoSize = NULL;
	UINT CopyFlags = static_cast<UINT>(Param_CopyFlags);

	//Recupera o ponteiro para a interface de recurso
	Resultado = Param_DestRecursoTiled->RecuperarPonteiro((LPVOID*)&pRecursoDestino);
	...
	//Converte as estruturas.
	pCoordnadasInicio = Util.ConverterD3D11_TILED_RESOURCE_COORDINATEManaged_ToUnManaged(Param_CoordernadasInicioRecursoTiled);
	pRegiaoSize = Util.ConverterD3D11_TILE_REGION_SIZEManaged_ToUnManaged(Param_SizeReigoesRecursoTiled);

	//Chama o método para realizar a operação.
	Param_MyPointerWork->UpdateTiles(pRecursoDestino, pCoordnadasInicio, pRegiaoSize, pBufferOrigem, CopyFlags);
...
Done:;
	...
	DeletarEstruturaSafe(&pCoordnadasInicio);
	DeletarEstruturaSafe(&pRegiaoSize);
...
CarenResult Shared_Direct3D11DeviceContext::ClearView(
	...
	UInt32 Param_QuantidadeItems)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->ClearView(pVisaoRecurso, pRGBAColor, pArrayRetangulos, static_cast<UINT>(Param_QuantidadeItems));
...
CarenResult Shared_Direct3D11DeviceContext::CopySubresourceRegion1(
	ID3D11DeviceContext1* Param_MyPointerWork,
	...
	CA_D3D11_COPY_FLAGS Param_CopyFlags)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CopySubresourceRegion1(
		pRecursoDestino,
...
CarenResult Shared_Direct3D11DeviceContext::CSGetConstantBuffers1(
	ID3D11DeviceContext1* Param_MyPointerWork,
	...
	[Out] cli::array<UInt32>^% Param_Out_MatrizNumeroConstantes)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CSGetConstantBuffers1(Param_StartSlot, Param_NumeroBuffers, VetorDataInterface.data(), pOutArrayMatrizInicioConstantes, pOutArrayNumeroConstantes);
...
CarenResult Shared_Direct3D11DeviceContext::CSSetConstantBuffers1(
	ID3D11DeviceContext1* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	...
	cli::array<UInt32>^ Param_MatrizNumeroConstantes)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CSSetConstantBuffers1(Param_StartSlot, Param_NumeroBuffers, VectorInterfacesBuffers.data(), pArrayInicioConstantes, pArrayNumeroConstantes);
...
CarenResult Shared_Direct3D11DeviceContext::DiscardResource(ID3D11DeviceContext1* Param_MyPointerWork, ICarenD3D11Resource^ Param_Recurso)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DiscardResource(pRecurso);
...
CarenResult Shared_Direct3D11DeviceContext::DiscardView(ID3D11DeviceContext1* Param_MyPointerWork, ICarenD3D11View^ Param_Visão)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DiscardView(pView);
...
CarenResult Shared_Direct3D11DeviceContext::DiscardView1(ID3D11DeviceContext1* Param_MyPointerWork, ICarenD3D11View^ Param_Visão, cli::array<CA_RECT^>^ Param_MatrizRetangulos, UInt32 Param_NumeroRetangulos)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DiscardView1(pView, pArrayRetangulos, Param_NumeroRetangulos);
...
/// que é especificada na matriz (Param_Out_MatrizInicioConstante). O tempo de execução define (Param_Out_MatrizNumeroConstantes) para NULO se não especificar o número de 
/// constantes em cada buffer.</param>
CarenResult Shared_Direct3D11DeviceContext::DSGetConstantBuffers1(
	ID3D11DeviceContext1* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	[Out] cli::array<ICarenD3D11Buffer^>^% Param_Out_MatrizBuffers,
	...
	[Out] cli::array<UInt32>^% Param_Out_MatrizNumeroConstantes)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DSGetConstantBuffers1(Param_StartSlot, Param_NumeroBuffers, VetorDataInterface.data(), pOutArrayMatrizInicioConstantes, pOutArrayNumeroConstantes);
...
/// de constantes que estão contidas no buffer constante que o sombreador usa. Cada número de constantes parte de sua respectiva compensação que é especificada na matriz 
/// (Param_MatrizInicioConstante). Cada número de constantes deve ser um múltiplo de 16 constantes, na faixa [0..4096].</param>
CarenResult Shared_Direct3D11DeviceContext::DSSetConstantBuffers1(
	ID3D11DeviceContext1* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	cli::array<ICarenD3D11Buffer^>^ Param_MatrizBuffers,
	...
	cli::array<UInt32>^ Param_MatrizNumeroConstantes)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DSSetConstantBuffers1(Param_StartSlot, Param_NumeroBuffers, VectorInterfacesBuffers.data(), pArrayInicioConstantes, pArrayNumeroConstantes);
...
/// que é especificada na matriz (Param_Out_MatrizInicioConstante). O tempo de execução define (Param_Out_MatrizNumeroConstantes) para NULO se não especificar o número de 
/// constantes em cada buffer.</param>
CarenResult Shared_Direct3D11DeviceContext::GSGetConstantBuffers1(
	ID3D11DeviceContext1* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	[Out] cli::array<ICarenD3D11Buffer^>^% Param_Out_MatrizBuffers,
	...
	[Out] cli::array<UInt32>^% Param_Out_MatrizNumeroConstantes)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GSGetConstantBuffers1(Param_StartSlot, Param_NumeroBuffers, VetorDataInterface.data(), pOutArrayMatrizInicioConstantes, pOutArrayNumeroConstantes);
...
/// de constantes que estão contidas no buffer constante que o sombreador usa. Cada número de constantes parte de sua respectiva compensação que é especificada na matriz 
/// (Param_MatrizInicioConstante). Cada número de constantes deve ser um múltiplo de 16 constantes, na faixa [0..4096].</param>
CarenResult Shared_Direct3D11DeviceContext::GSSetConstantBuffers1(
	ID3D11DeviceContext1* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	cli::array<ICarenD3D11Buffer^>^ Param_MatrizBuffers,
	...
	cli::array<UInt32>^ Param_MatrizNumeroConstantes)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GSSetConstantBuffers1(Param_StartSlot, Param_NumeroBuffers, VectorInterfacesBuffers.data(), pArrayInicioConstantes, pArrayNumeroConstantes);
...
/// que é especificada na matriz (Param_Out_MatrizInicioConstante). O tempo de execução define (Param_Out_MatrizNumeroConstantes) para NULO se não especificar o número de 
/// constantes em cada buffer.</param>
CarenResult Shared_Direct3D11DeviceContext::HSGetConstantBuffers1(
	ID3D11DeviceContext1* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	[Out] cli::array<ICarenD3D11Buffer^>^% Param_Out_MatrizBuffers,
	...
	[Out] cli::array<UInt32>^% Param_Out_MatrizNumeroConstantes)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->HSGetConstantBuffers1(Param_StartSlot, Param_NumeroBuffers, VetorDataInterface.data(), pOutArrayMatrizInicioConstantes, pOutArrayNumeroConstantes);
...
/// de constantes que estão contidas no buffer constante que o sombreador usa. Cada número de constantes parte de sua respectiva compensação que é especificada na matriz 
/// (Param_MatrizInicioConstante). Cada número de constantes deve ser um múltiplo de 16 constantes, na faixa [0..4096].</param>
CarenResult Shared_Direct3D11DeviceContext::HSSetConstantBuffers1(
	ID3D11DeviceContext1* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	cli::array<ICarenD3D11Buffer^>^ Param_MatrizBuffers,
	...
	cli::array<UInt32>^ Param_MatrizNumeroConstantes)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->HSSetConstantBuffers1(Param_StartSlot, Param_NumeroBuffers, VectorInterfacesBuffers.data(), pArrayInicioConstantes, pArrayNumeroConstantes);
...
/// que é especificada na matriz (Param_Out_MatrizInicioConstante). O tempo de execução define (Param_Out_MatrizNumeroConstantes) para NULO se não especificar o número de 
/// constantes em cada buffer.</param>
CarenResult Shared_Direct3D11DeviceContext::PSGetConstantBuffers1(
	ID3D11DeviceContext1* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	[Out] cli::array<ICarenD3D11Buffer^>^% Param_Out_MatrizBuffers,
	...
	[Out] cli::array<UInt32>^% Param_Out_MatrizNumeroConstantes)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->PSGetConstantBuffers1(Param_StartSlot, Param_NumeroBuffers, VetorDataInterface.data(), pOutArrayMatrizInicioConstantes, pOutArrayNumeroConstantes);
...
/// de constantes que estão contidas no buffer constante que o sombreador usa. Cada número de constantes parte de sua respectiva compensação que é especificada na matriz 
/// (Param_MatrizInicioConstante). Cada número de constantes deve ser um múltiplo de 16 constantes, na faixa [0..4096].</param>
CarenResult Shared_Direct3D11DeviceContext::PSSetConstantBuffers1(
	ID3D11DeviceContext1* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	cli::array<ICarenD3D11Buffer^>^ Param_MatrizBuffers,
	...
	cli::array<UInt32>^ Param_MatrizNumeroConstantes)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->PSSetConstantBuffers1(Param_StartSlot, Param_NumeroBuffers, VectorInterfacesBuffers.data(), pArrayInicioConstantes, pArrayNumeroConstantes);
...
CarenResult Shared_Direct3D11DeviceContext::SwapDeviceContextState(
	ID3D11DeviceContext1* Param_MyPointerWork,
	...
	[Out] ICarenD3DDeviceContextState^% Param_Out_EstadoAnterior)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->SwapDeviceContextState(pEstadoAtual, &pOutEstadoAnterior);
...
CarenResult Shared_Direct3D11DeviceContext::UpdateSubresource1(
	ID3D11DeviceContext1* Param_MyPointerWork,
	...
	CA_D3D11_COPY_FLAGS Param_CopyFlags)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->UpdateSubresource1(pRecursoDestino, Param_SubrecursoDestino, pBOX ? pBOX : NULL, pDadosOrigem, Param_TamanhoLinhaOrigem, Param_TamhoFatiaProdundidade, CopyFlags);
...
/// que é especificada na matriz (Param_Out_MatrizInicioConstante). O tempo de execução define (Param_Out_MatrizNumeroConstantes) para NULO se não especificar o número de 
/// constantes em cada buffer.</param>
CarenResult Shared_Direct3D11DeviceContext::VSGetConstantBuffers1(
	ID3D11DeviceContext1* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	[Out] cli::array<ICarenD3D11Buffer^>^% Param_Out_MatrizBuffers,
	...
	[Out] cli::array<UInt32>^% Param_Out_MatrizNumeroConstantes)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->VSGetConstantBuffers1(Param_StartSlot, Param_NumeroBuffers, VetorDataInterface.data(), pOutArrayMatrizInicioConstantes, pOutArrayNumeroConstantes);
...
/// de constantes que estão contidas no buffer constante que o sombreador usa. Cada número de constantes parte de sua respectiva compensação que é especificada na matriz 
/// (Param_MatrizInicioConstante). Cada número de constantes deve ser um múltiplo de 16 constantes, na faixa [0..4096].</param>
CarenResult Shared_Direct3D11DeviceContext::VSSetConstantBuffers1(
	ID3D11DeviceContext1* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	cli::array<ICarenD3D11Buffer^>^ Param_MatrizBuffers,
	...
	cli::array<UInt32>^ Param_MatrizNumeroConstantes)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->VSSetConstantBuffers1(Param_StartSlot, Param_NumeroBuffers, VectorInterfacesBuffers.data(), pArrayInicioConstantes, pArrayNumeroConstantes);
...
CarenResult Shared_Direct3D11DeviceContext::Begin(ID3D11DeviceContext* Param_MyPointerWork, ICarenD3D11Asynchronous^ Param_Async)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->Begin(pAsync);
...
CarenResult Shared_Direct3D11DeviceContext::ClearDepthStencilView(
	...
	Byte Param_Stencil)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->ClearDepthStencilView(pStencil, FlagClear, Param_Depth, Param_Stencil);
...
CarenResult Shared_Direct3D11DeviceContext::ClearRenderTargetView(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	Estruturas::CA_DXGI_RGBA^ Param_Cor)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->ClearRenderTargetView(pView, pRGBAColor);
...
/// </summary>
CarenResult Shared_Direct3D11DeviceContext::ClearState(ID3D11DeviceContext* Param_MyPointerWork)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);

	//Chama o método para realizar a operação.
	Param_MyPointerWork->ClearState();
...
CarenResult Shared_Direct3D11DeviceContext::ClearUnorderedAccessViewFloat(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	cli::array<float>^ Param_Valores)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->ClearUnorderedAccessViewFloat(pAcessView, pValores);
...
/// <param name="Param_UnorderedAccess">O ID3D11UnorderedAccessView para limpar.</param>
/// <param name="Param_Valores">Valores para copiar para canais correspondentes.</param>
CarenResult Shared_Direct3D11DeviceContext::ClearUnorderedAccessViewUint(
	ID3D11DeviceContext* Param_MyPointerWork,
	ICarenD3D11UnorderedAccessView^ Param_UnorderedAccess,
	cli::array<UInt32>^ Param_Valores)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->ClearUnorderedAccessViewUint(pAcessView, pValores);
...
CarenResult Shared_Direct3D11DeviceContext::CopyResource(
	ID3D11DeviceContext* Param_MyPointerWork,
	ICarenD3D11Resource^ Param_DestinoRecurso,
	ICarenD3D11Resource^ Param_OrigemRecurso)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CopyResource(pRecursoDestino, pRecursoOrigem);
...
CarenResult Shared_Direct3D11DeviceContext::CopyStructureCount(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	ICarenD3D11UnorderedAccessView^ Param_OrigemView)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CopyStructureCount(pBufferDestino, Param_DestinoAlignedOffsetByte, pAcessView);
...
/// <param name="Param_OrigemBox">Uma estrutura(CAIXA 3D) que define o sub-recurso de fonte que pode ser copiado. Se nulo, o sub-recurso fonte inteiro é copiado. A caixa deve caber dentro da fonte 
/// de recurso.</param>
CarenResult Shared_Direct3D11DeviceContext::CopySubresourceRegion(
	ID3D11DeviceContext* Param_MyPointerWork,
	ICarenD3D11Resource^ Param_DestinoRecurso,
	UInt32 Param_IndiceSubrecrusoDestino,
	UInt32 Param_DestX,
...
)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CopySubresourceRegion(pRecursoDestino, Param_IndiceSubrecrusoDestino, Param_DestX, Param_DestY, Param_DestZ, pRecursoOrigem, Param_IndiceSubrecrusoFonte, pCaixa ? pCaixa : NULL);
...
CarenResult Shared_Direct3D11DeviceContext::CSGetConstantBuffers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	...
	[Out] cli::array<ICarenD3D11Buffer^>^% Param_Out_BuffersConstantes)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CSGetConstantBuffers(Param_StartSlot, Param_NumerosBuffers, VetorDataInterface.data());
...
CarenResult Shared_Direct3D11DeviceContext::CSGetSamplers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	...
	[Out] cli::array<ICarenD3D11SamplerState^>^% Param_Out_SamplersState)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CSGetSamplers(Param_StartSlot, Param_NumeroSamplers, VetorDataInterface.data());
...
CarenResult Shared_Direct3D11DeviceContext::CSGetShader(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] UInt32% Param_Out_QuantidadeClass)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CSGetShader(&pComputeShader, VetorInterfaces.data(), &CountInterfacesInArray);
...
CarenResult Shared_Direct3D11DeviceContext::CSGetShaderResources(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	...
	[Out] cli::array<ICarenD3D11ShaderResourceView^>^% Param_Out_MatrizShaderRecursoSombreador)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CSGetShaderResources(Param_StartSlot, Param_NumeroRecursos, VectorResourceView.data());
...
CarenResult Shared_Direct3D11DeviceContext::CSGetUnorderedAccessViews(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	...
	[Out] cli::array<ICarenD3D11UnorderedAccessView^>^% Param_Out_MatrizInterfacesNaoOrdenadas)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CSGetUnorderedAccessViews(Param_StartSlot, Param_NumeroUAVs, VectorUnordered.data());
...
/// <param name="Param_NumeroBuffers">Número de buffers para definir (varia de 0 a D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_MatrizBuffers">Matriz de buffers constantes a ser dada ao dispositivo.</param>
CarenResult Shared_Direct3D11DeviceContext::CSSetConstantBuffers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	cli::array<ICarenD3D11Buffer^>^ Param_MatrizBuffers)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CSSetConstantBuffers(Param_StartSlot, Param_NumeroBuffers, VectorInterfaces.data());
...
CarenResult Shared_Direct3D11DeviceContext::CSSetSamplers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroSamplers,
	cli::array<ICarenD3D11SamplerState^>^ Param_MatrizSamplers)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CSSetSamplers(Param_StartSlot, Param_NumeroSamplers, VectorInterfaces.data());
...
CarenResult Shared_Direct3D11DeviceContext::CSSetShader(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	UInt32 Param_QuantidadeClassInstances)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CSSetShader(pComputeShader ? pComputeShader : NULL, Param_QuantidadeClassInstances > 0 ? VectorInterfaces.data() : NULL, Param_QuantidadeClassInstances);
...
CarenResult Shared_Direct3D11DeviceContext::CSSetShaderResources(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroRecursos,
	cli::array<ICarenD3D11ShaderResourceView^>^ Param_MatrizShaderRecursoSombreador)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CSSetShaderResources(Param_StartSlot, Param_NumeroRecursos, VectorInterfaces.data());
...
CarenResult Shared_Direct3D11DeviceContext::CSSetUnorderedAccessViews(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroUAVs,
	...
	cli::array<UInt32>^ Param_MatrizInitialUAVsCount)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CSSetUnorderedAccessViews(Param_StartSlot, Param_NumeroUAVs, VectorInterfaces.data(), pUAVsMatriz ? pUAVsMatriz : NULL);
...
CarenResult Shared_Direct3D11DeviceContext::Dispatch(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	UInt32 Param_NumeroGrupoExpedidoZ)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->Dispatch(Param_NumeroGrupoExpedidoX, Param_NumeroGrupoExpedidoY, Param_NumeroGrupoExpedidoZ);
...
CarenResult Shared_Direct3D11DeviceContext::DispatchIndirect(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	UInt32 Param_AlinhamentoBytesOffsetForArgs)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DispatchIndirect(pBuffer, Param_AlinhamentoBytesOffsetForArgs);
...
CarenResult Shared_Direct3D11DeviceContext::Draw(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	UInt32 Param_LocalizacaoVertice)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->Draw(Param_NumeroVertices, Param_LocalizacaoVertice);
...
/// </summary>
CarenResult Shared_Direct3D11DeviceContext::DrawAuto(ID3D11DeviceContext* Param_MyPointerWork)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);

	//Chama o método para realizar a operação.
	Param_MyPointerWork->DrawAuto();
...
CarenResult Shared_Direct3D11DeviceContext::DrawIndexed(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	Int32 Param_BaseVerticeLocalizacao)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DrawIndexed(Param_NumeroIndices, Param_StartIndexLocalizacao, Param_BaseVerticeLocalizacao);
...
CarenResult Shared_Direct3D11DeviceContext::DrawIndexedInstanced(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	UInt32 Param_StartInstanciaLocalizacao)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DrawIndexedInstanced(Param_QuantidadeIndicesPorInstancia, Param_QuantidadeInstancias, Param_StartIndexLocalizacao, Param_BaseVerticeLocalizacao, Param_StartInstanciaLocalizacao);
...
CarenResult Shared_Direct3D11DeviceContext::DrawIndexedInstancedIndirect(
	ID3D11DeviceContext* Param_MyPointerWork,
	ICarenD3D11Buffer^ Param_BufferForArgs,
	UInt32 Param_AlinhamentoBytesOffsetForArgs)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DrawIndexedInstancedIndirect(pBuffer, Param_AlinhamentoBytesOffsetForArgs);
...
/// <param name="Param_StartInstanciaLocalizacao">Um valor acrescentado para cada índice antes de ler dados por instância de um buffer de vértice.</param>			
CarenResult Shared_Direct3D11DeviceContext::DrawInstanced(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	UInt32 Param_StartInstanciaLocalizacao)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DrawInstanced(Param_QuantidadeVerticiesPorInstancia, Param_QuantidadeInstancias, Param_StartVerticeLocalizacao, Param_StartInstanciaLocalizacao);
...
CarenResult Shared_Direct3D11DeviceContext::DrawInstancedIndirect(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	UInt32 Param_DeslocamentoDados)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DrawIndexedInstancedIndirect(pBuffer, Param_DeslocamentoDados);
...
/// <param name="Param_NumeroBuffers">Número de buffers para recuperar (varia de 0 a D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_Out_MatrizBuffers">Retorna uma matriz que contém as interfaces para o Buffers constantes.</param>
CarenResult Shared_Direct3D11DeviceContext::DSGetConstantBuffers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	[Out] cli::array<ICarenD3D11Buffer^>^% Param_Out_MatrizBuffers)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DSGetConstantBuffers(Param_StartSlot, Param_NumeroBuffers, VetorDataInterface.data());
...
CarenResult Shared_Direct3D11DeviceContext::DSGetSamplers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	...
	[Out] cli::array<ICarenD3D11SamplerState^>^% Param_Out_MatrizAmostradoresEstado)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DSGetSamplers(Param_StartSlot, Param_Amostradores, VetorDataInterface.data());
...
CarenResult Shared_Direct3D11DeviceContext::DSGetShader(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] UInt32% Param_Out_ElementosMatriz)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DSGetShader(&pDomainShader, VetorInterfaces.data(), &CountInterfacesInArray);
...
CarenResult Shared_Direct3D11DeviceContext::DSGetShaderResources(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	...
	[Out] cli::array<ICarenD3D11ShaderResourceView^>^% Param_Out_MatrizRecursoShader)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DSGetShaderResources(Param_StartSlot, Param_QuantidadeRecursos, VectorResourceView.data());
...
/// <param name="Param_NumeroBuffers">Número de buffers para definir (varia de 0 a D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_MatrizBuffers">Matriz de buffers constantes a ser dada ao dispositivo.</param>
CarenResult Shared_Direct3D11DeviceContext::DSSetConstantBuffers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	cli::array<ICarenD3D11Buffer^>^ Param_MatrizBuffers)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DSSetConstantBuffers(Param_StartSlot, Param_NumeroBuffers, VectorInterfaces.data());
...
CarenResult Shared_Direct3D11DeviceContext::DSSetSamplers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_Amostradores,
	cli::array<ICarenD3D11SamplerState^>^ Param_MatrizAmostradoresEstado)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DSSetSamplers(Param_StartSlot, Param_Amostradores, VectorInterfaces.data());
...
CarenResult Shared_Direct3D11DeviceContext::DSSetShader(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	UInt32 Param_ElementosMatriz)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DSSetShader(pDomainShader ? pDomainShader : NULL, Param_ElementosMatriz > 0 ? VectorInterfaces.data() : NULL, Param_ElementosMatriz);
...
CarenResult Shared_Direct3D11DeviceContext::DSSetShaderResources(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_QuantidadeRecursos,
	cli::array<ICarenD3D11ShaderResourceView^>^ Param_MatrizRecursoShader)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->DSSetShaderResources(Param_StartSlot, Param_QuantidadeRecursos, VectorInterfaces.data());
...
CarenResult Shared_Direct3D11DeviceContext::End(ID3D11DeviceContext* Param_MyPointerWork, ICarenD3D11Asynchronous^ Param_Async)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->End(pAsync);
...
CarenResult Shared_Direct3D11DeviceContext::ExecuteCommandList(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	Boolean Param_RestaurarEstadoContexto)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->ExecuteCommandList(pCommandList, Param_RestaurarEstadoContexto ? TRUE : FALSE);
...
CarenResult Shared_Direct3D11DeviceContext::FinishCommandList(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] ICarenD3D11CommandList^% Param_Out_FilaComandos)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->FinishCommandList(Param_RestDeferidoEstadoContexto ? TRUE : FALSE, &pCommandList);
...
/// </summary>
CarenResult Shared_Direct3D11DeviceContext::Flush(ID3D11DeviceContext* Param_MyPointerWork)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);

	//Chama o método para realizar a operação.
	Param_MyPointerWork->Flush();
...
CarenResult Shared_Direct3D11DeviceContext::GenerateMips(ID3D11DeviceContext* Param_MyPointerWork, ICarenD3D11ShaderResourceView^ Param_RecursoSombreador)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GenerateMips(pRecursoView);
...
/// uso futuro.</param>
CarenResult Shared_Direct3D11DeviceContext::GetContextFlags(ID3D11DeviceContext* Param_MyPointerWork, [Out] UInt32% Param_Out_ContextFlags)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);

	//Chama o método para realizar a operação.
	Param_Out_ContextFlags = Param_MyPointerWork->GetContextFlags();
...
CarenResult Shared_Direct3D11DeviceContext::GetData(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	Enumeracoes::CA_D3D11_ASYNC_GETDATA_FLAG Param_FlagsGetData)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetData(pAsync, pData ? pData : NULL, Param_TamanhoDados, FlagsGetData);
...
CarenResult Shared_Direct3D11DeviceContext::GetPredication(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] Boolean% Param_Out_ValuePredicado)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GetPredication(&pPredicate, &ValuePredicado);
...
CarenResult Shared_Direct3D11DeviceContext::GetResourceMinLOD(ID3D11DeviceContext* Param_MyPointerWork, ICarenD3D11Resource^ Param_Recurso, [Out] float% Param_Out_LODMinimo)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_Out_LODMinimo = Param_MyPointerWork->GetResourceMinLOD(pRecurso);
...
CarenResult Shared_Direct3D11DeviceContext::GetType(ID3D11DeviceContext* Param_MyPointerWork, [Out] Enumeracoes::CA_D3D11_DEVICE_CONTEXT_TYPE% Param_Out_ContextoDispositivo)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);

	//Chama o método para realizar a operação.
	Param_Out_ContextoDispositivo = static_cast<CA_D3D11_DEVICE_CONTEXT_TYPE>(Param_MyPointerWork->GetType());
...
CarenResult Shared_Direct3D11DeviceContext::GSGetConstantBuffers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	[Out] cli::array<ICarenD3D11Buffer^>^% Param_Out_MatrizBuffers)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GSGetConstantBuffers(Param_StartSlot, Param_NumeroBuffers, VetorDataInterface.data());
...
CarenResult Shared_Direct3D11DeviceContext::GSGetSamplers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	...
	[Out] cli::array<ICarenD3D11SamplerState^>^% Param_Out_MatrizEstadoAmostrado)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GSGetSamplers(Param_StartSlot, Param_NumeroAmostradores, VetorDataInterface.data());
...
CarenResult Shared_Direct3D11DeviceContext::GSGetShader(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] UInt32% Param_Out_QuantidadeInstancias)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GSGetShader(&pGeometryShader, VetorInterfaces.data(), &CountInterfacesInArray);
...
CarenResult Shared_Direct3D11DeviceContext::GSGetShaderResources(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	...
	[Out] cli::array<ICarenD3D11ShaderResourceView^>^% Param_Out_MatrizVisualizadoresSombreador)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GSGetShaderResources(Param_StartSlot, Param_NumeroViews, VectorResourceView.data());
...
CarenResult Shared_Direct3D11DeviceContext::GSSetConstantBuffers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	cli::array<ICarenD3D11Buffer^>^ Param_MatrizBuffers)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GSSetConstantBuffers(Param_StartSlot, Param_NumeroBuffers, VectorInterfaces.data());
...
CarenResult Shared_Direct3D11DeviceContext::GSSetSamplers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroAmostradores,
	cli::array<ICarenD3D11SamplerState^>^ Param_MatrizAmostradoresEstado)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GSSetSamplers(Param_StartSlot, Param_NumeroAmostradores, VectorInterfaces.data());
...
CarenResult Shared_Direct3D11DeviceContext::GSSetShader(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	UInt32 Param_NumeroInstanciasClasse)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GSSetShader(pGeometryShader ? pGeometryShader : NULL, Param_NumeroInstanciasClasse > 0 ? VectorInterfaces.data() : NULL, Param_NumeroInstanciasClasse);
...
CarenResult Shared_Direct3D11DeviceContext::GSSetShaderResources(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroViews,
	cli::array<ICarenD3D11ShaderResourceView^>^ Param_MatrizVisualizadoresSombreador)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GSSetShaderResources(Param_StartSlot, Param_NumeroViews, VectorInterfaces.data());
...
/// <param name="Param_NumeroBuffers">Número de buffers a serem recuperados (varia de 0 a D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_Out_MatrizBuffers">Retorna uma matriz de ponteiros de interface de buffer constante.</param>
CarenResult Shared_Direct3D11DeviceContext::HSGetConstantBuffers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	[Out] cli::array<ICarenD3D11Buffer^>^% Param_Out_MatrizBuffers)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->HSGetConstantBuffers(Param_StartSlot, Param_NumeroBuffers, VetorDataInterface.data());
...
/// <param name="Param_NumeroAmostradores">Número de (Samplers) para obter de um contexto de dispositivo. Cada estágio de pipeline tem um total de 16 slots disponíveis (varia de 0 a D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_Out_MatrizEstadoAmostrado">Retorna uma matriz com interfaces do amostrador de estado.</param>
CarenResult Shared_Direct3D11DeviceContext::HSGetSamplers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroAmostradores,
	[Out] cli::array<ICarenD3D11SamplerState^>^% Param_Out_MatrizEstadoAmostrado)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->HSGetSamplers(Param_StartSlot, Param_NumeroAmostradores, VetorDataInterface.data());
...
/// <param name="Param_Out_MatrizInstanciasClasse">Retorna uma matriz com interfaces de Instancias de classe.</param>
/// <param name="Param_Out_QuantidadeInstancias">Retorna a quantidade de de instancias de classe na matriz do parametro (Param_Out_MatrizInstanciasClasse).</param>
CarenResult Shared_Direct3D11DeviceContext::HSGetShader(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] UInt32% Param_Out_QuantidadeInstancias)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->HSGetShader(&pHullShader, VetorInterfaces.data(), &CountInterfacesInArray);
...
/// <param name="Param_NumeroViews">O número de recursos para obter do dispositivo. Até um máximo de 128 slots estão disponíveis para recursos de sombreador (varia de 0 a D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_Out_MatrizVisualizadoresSombreador">Matriz de interfaces de visualização de recursos do sombreador a serem retornadas pelo dispositivo.</param>
CarenResult Shared_Direct3D11DeviceContext::HSGetShaderResources(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroViews,
	[Out] cli::array<ICarenD3D11ShaderResourceView^>^% Param_Out_MatrizVisualizadoresSombreador)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->HSGetShaderResources(Param_StartSlot, Param_NumeroViews, VectorResourceView.data());
...
/// <param name="Param_NumeroBuffers">Número de buffers a definir (varia de 0 a D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - Param_StartSlot).</param> 
/// <param name="Param_MatrizBuffers">Matriz de buffers constantes sendo fornecidos ao dispositivo.</param> 
CarenResult Shared_Direct3D11DeviceContext::HSSetConstantBuffers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	cli::array<ICarenD3D11Buffer^>^ Param_MatrizBuffers)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->HSSetConstantBuffers(Param_StartSlot, Param_NumeroBuffers, VectorInterfaces.data());
...
/// <param name="Param_NumeroAmostradores">Número de amostradores na matriz. Cada estágio de pipeline tem um total de 16 slots disponíveis (varia de 0 a D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_MatrizAmostradoresEstado">Uma matriz de interfaces de estado do amostrador.</param>
CarenResult Shared_Direct3D11DeviceContext::HSSetSamplers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroAmostradores,
	cli::array<ICarenD3D11SamplerState^>^ Param_MatrizAmostradoresEstado)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->HSSetSamplers(Param_StartSlot, Param_NumeroAmostradores, VectorInterfaces.data());
...
/// <param name="Param_MatrizInstanciaClasse">Uma matriz de interfaces de instância de classe. Cada interface usada por um sombreador deve ter uma instância de classe correspondente ou o shader será desativado. Defina como NULO se o sombreador não usar nenhuma interface.</param>
/// <param name="Param_NumeroInstanciasClasse">O numero de (ICarenD3D11ClassInstance) dentro da matriz (Param_MatrizInstanciaClasse).</param>
CarenResult Shared_Direct3D11DeviceContext::HSSetShader(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	UInt32 Param_NumeroInstanciasClasse)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->HSSetShader(pHullShader ? pHullShader : NULL, Param_NumeroInstanciasClasse > 0 ? VectorInterfaces.data() : NULL, Param_NumeroInstanciasClasse);
...
/// <param name="Param_NumeroViews">Número de recursos do shader a serem definidos. Até um máximo de 128 slots estão disponíveis para recursos de sombreador (o intervalo é de 0 a D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_MatrizVisualizadoresSombreador">Uma matriz que contém os Visualizadores de recurso do sombreador para serem definidos no Disposiivo.</param>
CarenResult Shared_Direct3D11DeviceContext::HSSetShaderResources(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroViews,
	cli::array<ICarenD3D11ShaderResourceView^>^ Param_MatrizVisualizadoresSombreador)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->HSSetShaderResources(Param_StartSlot, Param_NumeroViews, VectorInterfaces.data());
...
CarenResult Shared_Direct3D11DeviceContext::IAGetIndexBuffer(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] UInt32% Param_Out_Deslocamento)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->IAGetIndexBuffer(&pBuffer, &FormatoDadosBuffer, &DeslocamentoBuffer);
...
CarenResult Shared_Direct3D11DeviceContext::IAGetInputLayout(ID3D11DeviceContext* Param_MyPointerWork, [Out] ICarenD3D11InputLayout^% Param_Out_InputLayout)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->IAGetInputLayout(&pInputLayout);
...
CarenResult Shared_Direct3D11DeviceContext::IAGetPrimitiveTopology(ID3D11DeviceContext* Param_MyPointerWork, [Out] Enumeracoes::CA_D3D11_PRIMITIVE_TOPOLOGY% Param_Out_TipoPrimitivo)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->IAGetPrimitiveTopology(&PrimiteTopology);
...
CarenResult Shared_Direct3D11DeviceContext::IAGetVertexBuffers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	...
	[Out] cli::array<UInt32>^% Param_Out_MatrizDeslocamentos)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->IAGetVertexBuffers(Param_StartSlot, Param_NumeroBuffers, VectorInterfaceBuffer.data(), pMatrizStrides, pMatrizDeslocamentos);
...
CarenResult Shared_Direct3D11DeviceContext::IASetIndexBuffer(
	ID3D11DeviceContext* Param_MyPointerWork,
	ICarenD3D11Buffer^ Param_Buffer,
	...
	UInt32 Param_Deslocamento)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->IASetIndexBuffer(pBuffer, FormatoBuffer, Param_Deslocamento);
...
CarenResult Shared_Direct3D11DeviceContext::IASetInputLayout(ID3D11DeviceContext* Param_MyPointerWork, ICarenD3D11InputLayout^ Param_InputLayout)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->IASetInputLayout(pInputLayout);
...
CarenResult Shared_Direct3D11DeviceContext::IASetPrimitiveTopology(ID3D11DeviceContext* Param_MyPointerWork, Enumeracoes::CA_D3D11_PRIMITIVE_TOPOLOGY Param_TipoPrimitivo)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->IASetPrimitiveTopology(PrimitiveTopo);
...
CarenResult Shared_Direct3D11DeviceContext::IASetVertexBuffers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	...
	cli::array<UInt32>^ Param_MatrizDeslocamentos)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->IASetVertexBuffers(Param_StartSlot, Param_NumeroBuffers, VectorInterfaceBuffer.data(), pMatrizStrides, pMatrizDeslocamentos);
...
CarenResult Shared_Direct3D11DeviceContext::Map(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] Estruturas::CA_D3D11_MAPPED_SUBRESOURCE^% Param_Out_RecrusoMapeado)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->Map(pRecurso, Param_IndiceSubRecurso, PermissaoAcesso, Flags, pEstruturaData);
...
CarenResult Shared_Direct3D11DeviceContext::OMGetBlendState(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] UInt32% Param_Out_SampleMask)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->OMGetBlendState(&pBlendState, pMatrizMisturaRGBA, &SampleMask);

	//Verifica se o ponteiro é valido
	if (ObjetoValido(pBlendState))
	{
		//Cria a interface que vai conter o ponteiro da interface a ser retornada.
		Param_Out_EstadoMistura = gcnew CarenD3D11BlendState();

		//Define o ponteiro de trabalho
		Param_Out_EstadoMistura->AdicionarPonteiro(pBlendState);
	}
...
CarenResult Shared_Direct3D11DeviceContext::OMGetDepthStencilState(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] UInt32% Param_Out_StencilRef)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->OMGetDepthStencilState(&pStencilState, &StencilRef);
...
CarenResult Shared_Direct3D11DeviceContext::OMGetRenderTargets(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_NumeroViews,
	...
	[Out] ICarenD3D11DepthStencilView^% Param_Out_DepthStencilView)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->OMGetRenderTargets(Param_NumeroViews, Param_IgnorarRecuperaçãoDestino ? NULL : VectorInterfacesTargetView.data(), Param_IgnorarRecuperaçãoModoExibição ? NULL : &pStencilView);
...
CarenResult Shared_Direct3D11DeviceContext::OMGetRenderTargetsAndUnorderedAccessViews(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] cli::array<ICarenD3D11UnorderedAccessView^>^% Param_Out_MatrizUnorderedAccessViews)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->OMGetRenderTargetsAndUnorderedAccessViews(
		Param_NumeroRTVs,
...
CarenResult Shared_Direct3D11DeviceContext::OMSetBlendState(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	UInt32 Param_SampleMask)
{
	...
	if (ObjetoGerenciadoValido(Param_EstadoMistura))
	{
		//Recupera o ponteiro para a interface.
		Resultado = Param_EstadoMistura->RecuperarPonteiro((LPVOID*)&pBlendState);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->OMSetBlendState(pBlendState ? NULL : pBlendState, pFatoresMistura ? pFatoresMistura : NULL, Param_SampleMask);
...
CarenResult Shared_Direct3D11DeviceContext::OMSetDepthStencilState(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	UInt32 Param_StencilRef)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->OMSetDepthStencilState(pDepthStencil ? pDepthStencil : NULL, Param_StencilRef);
...
CarenResult Shared_Direct3D11DeviceContext::OMSetRenderTargets(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_NumeroViews,
	...
	ICarenD3D11DepthStencilView^ Param_DepthStencilView)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->OMSetRenderTargets(Param_NumeroViews, ObjetoGerenciadoValido(Param_MatizRenderTargetViews) ? ppMatrizRendersDestino.data() : NULL, pDepthStencilView ? pDepthStencilView : NULL);
...
CarenResult Shared_Direct3D11DeviceContext::OMSetRenderTargetsAndUnorderedAccessViews(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_NumeroRTVs,
	...
	cli::array<UInt32>^ Param_MatrizUAVInitialCounts)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->OMSetRenderTargetsAndUnorderedAccessViews(
		Param_NumeroRTVs,
...
/// <param name="Param_NumeroBuffers">Número de buffers a serem recuperados (varia de 0 a D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_Out_MatrizBuffers">Retorna uma matriz de ponteiros de interface de buffer constante.</param>
CarenResult Shared_Direct3D11DeviceContext::PSGetConstantBuffers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	[Out] cli::array<ICarenD3D11Buffer^>^% Param_Out_MatrizBuffers)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->PSGetConstantBuffers(Param_StartSlot, Param_NumeroBuffers, VetorDataInterface.data());
...
/// <param name="Param_NumeroAmostradores">Número de (Samplers) para obter de um contexto de dispositivo. Cada estágio de pipeline tem um total de 16 slots disponíveis (varia de 0 a D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_Out_MatrizEstadoAmostrado">Retorna uma matriz com interfaces do amostrador de estado.</param>
CarenResult Shared_Direct3D11DeviceContext::PSGetSamplers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroAmostradores,
	[Out] cli::array<ICarenD3D11SamplerState^>^% Param_Out_MatrizEstadoAmostrado)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->PSGetSamplers(Param_StartSlot, Param_NumeroAmostradores, VetorDataInterface.data());
...
/// <param name="Param_Out_MatrizInstanciasClasse">Retorna uma matriz com interfaces de Instancias de classe.</param>
/// <param name="Param_Out_QuantidadeInstancias">Retorna a quantidade de de instancias de classe na matriz do parametro (Param_Out_MatrizInstanciasClasse).</param>
CarenResult Shared_Direct3D11DeviceContext::PSGetShader(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] UInt32% Param_Out_QuantidadeInstancias)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->PSGetShader(&pPixelShader, VetorInterfaces.data(), &CountInterfacesInArray);
...
/// <param name="Param_NumeroViews">O número de recursos para obter do dispositivo. Até um máximo de 128 slots estão disponíveis para recursos de sombreador (varia de 0 a D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_Out_MatrizVisualizadoresSombreador">Matriz de interfaces de visualização de recursos do sombreador a serem retornadas pelo dispositivo.</param>
CarenResult Shared_Direct3D11DeviceContext::PSGetShaderResources(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroViews,
	[Out] cli::array<ICarenD3D11ShaderResourceView^>^% Param_Out_MatrizVisualizadoresSombreador)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->PSGetShaderResources(Param_StartSlot, Param_NumeroViews, VectorResourceView.data());
...
/// <param name="Param_NumeroBuffers">Número de buffers a definir (varia de 0 a D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - Param_StartSlot).</param> 
/// <param name="Param_MatrizBuffers">Matriz de buffers constantes sendo fornecidos ao dispositivo.</param>
CarenResult Shared_Direct3D11DeviceContext::PSSetConstantBuffers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	cli::array<ICarenD3D11Buffer^>^ Param_MatrizBuffers)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->PSSetConstantBuffers(Param_StartSlot, Param_NumeroBuffers, VectorInterfaces.data());
...
/// <param name="Param_NumeroAmostradores">Número de amostradores na matriz. Cada estágio de pipeline tem um total de 16 slots disponíveis (varia de 0 a D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_MatrizAmostradoresEstado">Uma matriz de interfaces de estado do amostrador.</param>
CarenResult Shared_Direct3D11DeviceContext::PSSetSamplers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroAmostradores,
	cli::array<ICarenD3D11SamplerState^>^ Param_MatrizAmostradoresEstado)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->PSSetSamplers(Param_StartSlot, Param_NumeroAmostradores, VectorInterfaces.data());
...
/// <param name="Param_MatrizInstanciaClasse">Uma matriz de interfaces de instância de classe. Cada interface usada por um sombreador deve ter uma instância de classe correspondente ou o shader será desativado. Defina como NULO se o sombreador não usar nenhuma interface.</param>
/// <param name="Param_NumeroInstanciasClasse">O numero de (ICarenD3D11ClassInstance) dentro da matriz (Param_MatrizInstanciaClasse).</param>
CarenResult Shared_Direct3D11DeviceContext::PSSetShader(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	UInt32 Param_NumeroInstanciasClasse)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->PSSetShader(pPixelShader ? pPixelShader : NULL, Param_NumeroInstanciasClasse > 0 ? VectorInterfaces.data() : NULL, Param_NumeroInstanciasClasse);
...
/// <param name="Param_NumeroViews">Número de recursos do shader a serem definidos. Até um máximo de 128 slots estão disponíveis para recursos de sombreador (o intervalo é de 0 a D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_MatrizVisualizadoresSombreador">Uma matriz que contém os Visualizadores de recurso do sombreador para serem definidos no Disposiivo.</param>
CarenResult Shared_Direct3D11DeviceContext::PSSetShaderResources(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroViews,
	cli::array<ICarenD3D11ShaderResourceView^>^ Param_MatrizVisualizadoresSombreador)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->PSSetShaderResources(Param_StartSlot, Param_NumeroViews, VectorInterfaces.data());
...
CarenResult Shared_Direct3D11DeviceContext::ResolveSubresource(
	ID3D11DeviceContext* Param_MyPointerWork,
	ICarenD3D11Resource^ Param_RecursoDestino,
	...
	Enumeracoes::CA_DXGI_FORMAT Param_DXGIFormato)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->ResolveSubresource(pRecursoDestino, Param_IndiceSubRecursoDestino, pRecursoOrigem, Param_IndiceSubRecursoOrigem, FormatoDXGI);
...
CarenResult Shared_Direct3D11DeviceContext::RSGetScissorRects(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] cli::array<Estruturas::CA_RECT^>^% Param_Out_Rects)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->RSGetScissorRects(&NumeroRects, Param_ChecarQuantidade ? NULL : VectorRects.data());
...
CarenResult Shared_Direct3D11DeviceContext::RSGetState(
	ID3D11DeviceContext* Param_MyPointerWork, 
	[Out] ICarenD3D11RasterizerState^% Param_Out_EstadoRasterizador)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->RSGetState(&pRasterizador);
...
CarenResult Shared_Direct3D11DeviceContext::RSGetViewports(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] cli::array<Estruturas::CA_D3D11_VIEWPORT^>^% Param_Out_MatrizViewports)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->RSGetViewports(&NumeroViewPorts, Param_ObterQuantidadeViewports ? NULL : VectorViewPort.data());
...
CarenResult Shared_Direct3D11DeviceContext::RSSetScissorRects(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	cli::array<Estruturas::CA_RECT^>^ Param_MatrizRects)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->RSSetScissorRects(Param_NumeroRects, pRECTS);
...
CarenResult Shared_Direct3D11DeviceContext::RSSetState(ID3D11DeviceContext* Param_MyPointerWork, ICarenD3D11RasterizerState^ Param_EstadoRasterizador)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->RSSetState(pRasterizador);
...
CarenResult Shared_Direct3D11DeviceContext::RSSetViewports(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	cli::array<Estruturas::CA_D3D11_VIEWPORT^>^ Param_MatrizViewports)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->RSSetViewports(Param_NumeroViewports, pArrayViewPorts);
...
CarenResult Shared_Direct3D11DeviceContext::SetPredication(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	Boolean Param_ValorPredicado)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->SetPredication(pPredicado ? pPredicado : NULL, Param_ValorPredicado ? TRUE : FALSE);
...
CarenResult Shared_Direct3D11DeviceContext::SetResourceMinLOD(
	ID3D11DeviceContext* Param_MyPointerWork,
	ICarenD3D11Resource^ Param_Recurso,
	float Param_MinLOD)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->SetResourceMinLOD(pRecurso, Param_MinLOD);
...
CarenResult Shared_Direct3D11DeviceContext::SOGetTargets(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_NumeroBuffers,
	[Out] cli::array<ICarenD3D11Buffer^>^% Param_Out_MatrizBuffers)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->SOGetTargets(Param_NumeroBuffers, VectorBuffers.data());
...
/// em bytes.</param>
CarenResult Shared_Direct3D11DeviceContext::SOSetTargets(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_NumeroBuffers,
	cli::array<ICarenD3D11Buffer^>^ Param_MatrizBuffers,
	cli::array<UInt32>^ Param_DeslocamentosBuffers)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->SOSetTargets(Param_NumeroBuffers, VectorMatrizBuffer.data(), pDeslocamentos);
...
CarenResult Shared_Direct3D11DeviceContext::Unmap(
	ID3D11DeviceContext* Param_MyPointerWork,
	ICarenD3D11Resource^ Param_Recurso,
	UInt32 Param_SubrecursoInvalidado)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->Unmap(pRecurso, Param_SubrecursoInvalidado);
...
/// <param name="Param_TamanhoLinhaOrigem">(SrcRowPitch) - O tamanho de uma linha dos dados de origem.</param>
/// <param name="Param_TamhoFatiaProdundidade">(SrcDepthPitch) - O tamanho de uma fatia de profundidade dos dados de origem.</param>
CarenResult Shared_Direct3D11DeviceContext::UpdateSubresource(
	ID3D11DeviceContext* Param_MyPointerWork,
	ICarenD3D11Resource^ Param_RecursoDestino,
	UInt32 Param_SubrecursoDestino,
	Estruturas::CA_D3D11_BOX^ Param_Caixa,
	...
	UInt32 Param_TamhoFatiaProdundidade)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->UpdateSubresource(pRecursoDestino, Param_SubrecursoDestino, pBOX ? pBOX : NULL, pDadosOrigem, Param_TamanhoLinhaOrigem, Param_TamhoFatiaProdundidade);
...
/// <param name="Param_NumeroBuffers">Número de buffers a serem recuperados (varia de 0 a D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_Out_MatrizBuffers">Retorna uma matriz de ponteiros de interface de buffer constante.</param>
CarenResult Shared_Direct3D11DeviceContext::VSGetConstantBuffers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	[Out] cli::array<ICarenD3D11Buffer^>^% Param_Out_MatrizBuffers)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->VSGetConstantBuffers(Param_StartSlot, Param_NumeroBuffers, VetorDataInterface.data());
...
/// <param name="Param_NumeroAmostradores">Número de (Samplers) para obter de um contexto de dispositivo. Cada estágio de pipeline tem um total de 16 slots disponíveis (varia de 0 a D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_Out_MatrizEstadoAmostrado">Retorna uma matriz com interfaces do amostrador de estado.</param>
CarenResult Shared_Direct3D11DeviceContext::VSGetSamplers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroAmostradores,
	[Out] cli::array<ICarenD3D11SamplerState^>^% Param_Out_MatrizEstadoAmostrado)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->VSGetSamplers(Param_StartSlot, Param_NumeroAmostradores, VetorDataInterface.data());
...
/// <param name="Param_Out_MatrizInstanciasClasse">Retorna uma matriz com interfaces de Instancias de classe.</param>
/// <param name="Param_Out_QuantidadeInstancias">Retorna a quantidade de de instancias de classe na matriz do parametro (Param_Out_MatrizInstanciasClasse).</param>
CarenResult Shared_Direct3D11DeviceContext::VSGetShader(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	[Out] UInt32% Param_Out_QuantidadeInstancias)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->VSGetShader(&pVertexShader, VetorInterfaces.data(), &CountInterfacesInArray);
...
/// <param name="Param_NumeroViews">O número de recursos para obter do dispositivo. Até um máximo de 128 slots estão disponíveis para recursos de sombreador (varia de 0 a D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_Out_MatrizVisualizadoresSombreador">Matriz de interfaces de visualização de recursos do sombreador a serem retornadas pelo dispositivo.</param>
CarenResult Shared_Direct3D11DeviceContext::VSGetShaderResources(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroViews,
	[Out] cli::array<ICarenD3D11ShaderResourceView^>^% Param_Out_MatrizVisualizadoresSombreador)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->VSGetShaderResources(Param_StartSlot, Param_NumeroViews, VectorResourceView.data());
...
/// <param name="Param_NumeroBuffers">Número de buffers a definir (varia de 0 a D3D11_COMMONSHADER_CONSTANT_BUFFER_API_SLOT_COUNT - Param_StartSlot).</param> 
/// <param name="Param_MatrizBuffers">Matriz de buffers constantes sendo fornecidos ao dispositivo.</param> 
CarenResult Shared_Direct3D11DeviceContext::VSSetConstantBuffers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroBuffers,
	cli::array<ICarenD3D11Buffer^>^ Param_MatrizBuffers)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->VSSetConstantBuffers(Param_StartSlot, Param_NumeroBuffers, VectorInterfaces.data());
...
/// <param name="Param_NumeroAmostradores">Número de amostradores na matriz. Cada estágio de pipeline tem um total de 16 slots disponíveis (varia de 0 a D3D11_COMMONSHADER_SAMPLER_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_MatrizAmostradoresEstado">Uma matriz de interfaces de estado do amostrador.</param>
CarenResult Shared_Direct3D11DeviceContext::VSSetSamplers(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroAmostradores,
	cli::array<ICarenD3D11SamplerState^>^ Param_MatrizAmostradoresEstado)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->VSSetSamplers(Param_StartSlot, Param_NumeroAmostradores, VectorInterfaces.data());
...
/// <param name="Param_MatrizInstanciaClasse">Uma matriz de interfaces de instância de classe. Cada interface usada por um sombreador deve ter uma instância de classe correspondente ou o shader será desativado. Defina como NULO se o sombreador não usar nenhuma interface.</param>
/// <param name="Param_NumeroInstanciasClasse">O numero de (ICarenD3D11ClassInstance) dentro da matriz (Param_MatrizInstanciaClasse).</param>
CarenResult Shared_Direct3D11DeviceContext::VSSetShader(
	ID3D11DeviceContext* Param_MyPointerWork,
	...
	UInt32 Param_NumeroInstanciasClasse)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->VSSetShader(pVertexShader ? pVertexShader : NULL, Param_NumeroInstanciasClasse > 0 ? VectorInterfaces.data() : NULL, Param_NumeroInstanciasClasse);
...
/// <param name="Param_NumeroViews">Número de recursos do shader a serem definidos. Até um máximo de 128 slots estão disponíveis para recursos de sombreador (o intervalo é de 0 a D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT - Param_StartSlot).</param>
/// <param name="Param_MatrizVisualizadoresSombreador">Uma matriz que contém os Visualizadores de recurso do sombreador para serem definidos no Disposiivo.</param>
CarenResult Shared_Direct3D11DeviceContext::VSSetShaderResources(
	ID3D11DeviceContext* Param_MyPointerWork,
	UInt32 Param_StartSlot,
	UInt32 Param_NumeroViews,
	cli::array<ICarenD3D11ShaderResourceView^>^ Param_MatrizVisualizadoresSombreador)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->VSSetShaderResources(Param_StartSlot, Param_NumeroViews, VectorInterfaces.data());
