...
public:
	static operator CarenD2D1SpriteBatch^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD2D1SpriteBatch^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD2D1SpriteBatch();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID2D1SpriteBatch*>(Param_Pointer.ToPointer());
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD2D1SpriteBatch::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
