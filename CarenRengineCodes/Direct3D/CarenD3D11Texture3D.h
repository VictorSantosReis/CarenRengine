...
/// <summary>
/// (Concluido - Fase de Testes) - Classe responsável por acessar dados de Textura 3D, que são de memória estruturada.
/// </summary>
public ref class CarenD3D11Texture3D :public ICarenD3D11Texture3D
{
	/////////////////////////////////////////
	//Objeto gerenciado por essa interface.//
	/////////////////////////////////////////

	//Ponteiro para a interface (ID3D11Texture3D).
	ID3D11Texture3D* PonteiroTrabalho = NULL;
...
public:
	static operator CarenD3D11Texture3D^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD3D11Texture3D^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD3D11Texture3D();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID3D11Texture3D*>(Param_Pointer.ToPointer());
...
public:
	...
	/// </summary>
	/// <param name="Param_Out_CarenD3D11Texture3D">Recebe um ponteiro para a interface (Vazia).</param>
	static CarenResult CriarInstanciaVazia([Out] ICarenD3D11Texture3D^% Param_Out_CarenD3D11Texture3D)
	{
		//Variavel a ser retornada.
		CarenResult Resultado = CarenResult(E_FAIL, false);

		//Cria a interface
		Param_Out_CarenD3D11Texture3D = gcnew CarenD3D11Texture3D();
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD3D11Texture3D::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
...
public:
	...
	virtual CarenResult GetDesc([Out] Estruturas::CA_D3D11_TEXTURE3D_DESC^% Param_Out_PropsTextura3D);
