...
CarenResult CarenAudioClient2::AdicionarPonteiro(IntPtr Param_PonteiroNativo)
{
	...
	PonteiroTrabalho = reinterpret_cast<IAudioClient2*>(Param_PonteiroNativo.ToPointer());
...
CarenResult CarenAudioClient2::AdicionarPonteiro(LPVOID Param_PonteiroNativo)
{
	...
	//Converte o ponteiro para o tipo especifico da classe.
	PonteiroTrabalho = reinterpret_cast<IAudioClient2*>(Param_PonteiroNativo);
...
CarenResult CarenAudioClient2::SetEventHandle(IntPtr Param_EventHandle) {
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
