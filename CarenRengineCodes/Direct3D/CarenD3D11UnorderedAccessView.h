...
/// <summary>
/// (Concluido - Fase de Testes) - Classe responsável por representar uma visualização que especifica as partes de um recurso que o pipeline pode acessar durante a renderização.
/// </summary>
public ref class CarenD3D11UnorderedAccessView : public ICarenD3D11UnorderedAccessView
{
	/////////////////////////////////////////
	//Objeto gerenciado por essa interface.//
	/////////////////////////////////////////

	///Ponteiro para a interface (ID3D11UnorderedAccessView).
	ID3D11UnorderedAccessView* PonteiroTrabalho = NULL;
...
public:
	static operator CarenD3D11UnorderedAccessView^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD3D11UnorderedAccessView^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD3D11UnorderedAccessView();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID3D11UnorderedAccessView*>(Param_Pointer.ToPointer());
...
public:
	...
	/// </summary>
	/// <param name="Param_Out_CarenD3D11UnorderedAccessView">Recebe um ponteiro para a interface (Vazia).</param>
	static CarenResult CriarInstanciaVazia([Out] ICarenD3D11UnorderedAccessView^% Param_Out_CarenD3D11UnorderedAccessView)
	{
		//Variavel a ser retornada.
		CarenResult Resultado = CarenResult(E_FAIL, false);

		//Cria a interface
		Param_Out_CarenD3D11UnorderedAccessView = gcnew CarenD3D11UnorderedAccessView();
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD3D11UnorderedAccessView::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
...
public:
	...
	virtual CarenResult GetDesc([Out] Estruturas::CA_D3D11_UNORDERED_ACCESS_VIEW_DESC^% Param_Out_DescRecurso);
...
public:
	...
	/// (GetDesc) - Obtém uma descrição do recurso.
	/// </summary>
	/// <param name="Param_Out_Recurso">Retorna uma estrutura que contém a descrição de um recurso de visualização não ordenado.</param>
	virtual CarenResult GetResource([Out] ICarenD3D11Resource^% Param_Out_Recurso);
