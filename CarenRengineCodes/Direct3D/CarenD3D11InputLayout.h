...
/// <summary>
/// (Concluido - Fase de Testes) - Classe responsável por representar um (Layout de Entrada) que contém uma definição de como alimentar dados de vértice que é colocado na memória para a fase de (Montador de Entrada) do (Pipeline Gráfico).
/// </summary>
public ref class CarenD3D11InputLayout :public ICarenD3D11InputLayout
{
	/////////////////////////////////////////
...
public:
	...
	/// </summary>
	/// <param name="Param_Out_CarenD3D11InputLayout">Recebe um ponteiro para a interface (Vazia).</param>
	static CarenResult CriarInstanciaVazia([Out] ICarenD3D11InputLayout^% Param_Out_CarenD3D11InputLayout)
	{
		//Variavel a ser retornada.
		CarenResult Resultado = CarenResult(E_FAIL, false);

		//Cria a interface
		Param_Out_CarenD3D11InputLayout = gcnew CarenD3D11InputLayout();
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD3D11InputLayout::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
