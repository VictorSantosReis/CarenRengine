...
/// <summary>
/// (Concluido - Fase de Testes) - Classe responsável por receber uma reprodução dos comandos gravados em uma lista de comando. Um uso típico será para transformar a 
...
/// </summary>
public ref class CarenD2D1CommandSink : public ICarenD2D1CommandSink
{
	/////////////////////////////////////////
	//Objeto gerenciado por essa interface.//
	/////////////////////////////////////////

	//Ponteiro para a interface (ID2D1CommandSink).
	ID2D1CommandSink* PonteiroTrabalho = NULL;
...
public:
	...
	/// de trabalho. Se (Param_CriarInterface) for TRUE, o construtor vai criar uma implementação da interface(ID2D1CommandSink).
	/// </summary>
	...
	CarenD2D1CommandSink(Boolean Param_CriarInterface);
...
public:
	static operator CarenD2D1CommandSink^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD2D1CommandSink^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD2D1CommandSink(false);

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID2D1CommandSink*>(Param_Pointer.ToPointer());
...
public:
	...
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnBeginDraw^ OnBeginDraw;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (Clear).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnClear^ OnClear;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (DrawBitmap).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnDrawBitmap^ OnDrawBitmap;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (DrawGdiMetafile).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnDrawGdiMetafile^ OnDrawGdiMetafile;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (DrawGeometry).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnDrawGeometry^ OnDrawGeometry;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (DrawGlyphRun).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnDrawGlyphRun^ OnDrawGlyphRun;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (DrawImage).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnDrawImage^ OnDrawImage;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (DrawLine).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnDrawLine^ OnDrawLine;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (DrawRectangle).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnDrawRectangle^ OnDrawRectangle;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (EndDraw).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnEndDraw^ OnEndDraw;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (FillGeometry).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnFillGeometry^ OnFillGeometry;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (FillMesh).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnFillMesh^ OnFillMesh;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (FillOpacityMask).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnFillOpacityMask^ OnFillOpacityMask;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (FillRectangle).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnFillRectangle^ OnFillRectangle;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (PopAxisAlignedClip).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnPopAxisAlignedClip^ OnPopAxisAlignedClip;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (PopLayer).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnPopLayer^ OnPopLayer;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (PushAxisAlignedClip).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnPushAxisAlignedClip^ OnPushAxisAlignedClip;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (PushLayer).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnPushLayer^ OnPushLayer;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (SetAntialiasMode).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnSetAntialiasMode^ OnSetAntialiasMode;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (SetPrimitiveBlend).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnSetPrimitiveBlend^ OnSetPrimitiveBlend;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (SetTags).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnSetTags^ OnSetTags;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (SetTextAntialiasMode).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnSetTextAntialiasMode^ OnSetTextAntialiasMode;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (SetTextRenderingParams).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnSetTextRenderingParams^ OnSetTextRenderingParams;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (SetTransform).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnSetTransform^ OnSetTransform;

	/// <summary>
	/// Evento chamado para notificar a reprodução do comando (SetUnitMode).
	/// </summary>
	virtual event ICarenD2D1CommandSink::DelegateOnSetUnitMode^ OnSetUnitMode;
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD2D1CommandSink::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
	...
	//Métodos da interface(ICarenD2D1CommandSink)
public:
	/// <summary>
