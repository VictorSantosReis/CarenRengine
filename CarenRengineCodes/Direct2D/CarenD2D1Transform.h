...
public:
	static operator CarenD2D1Transform^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD2D1Transform^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD2D1Transform();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID2D1Transform*>(Param_Pointer.ToPointer());
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD2D1Transform::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
