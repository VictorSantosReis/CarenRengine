...
CarenResult CarenDXGIAdapter4::ConsultarInterface(String^ Param_Guid, ICaren^ Param_InterfaceSolicitada)
{
	...
	Hr = (reinterpret_cast<IDXGIAdapter4*>(PonteiroTrabalho))->QueryInterface(GuidInterface, (LPVOID*)&pInterfaceSolcitada);
...
CarenResult CarenDXGIAdapter4::AdicionarPonteiro(IntPtr Param_PonteiroNativo)
{
	...
	PonteiroTrabalho = reinterpret_cast<IDXGIAdapter4*>(Param_PonteiroNativo.ToPointer());
...
CarenResult CarenDXGIAdapter4::AdicionarPonteiro(LPVOID Param_PonteiroNativo)
{
	...
	//Converte o ponteiro para o tipo especifico da classe.
	PonteiroTrabalho = reinterpret_cast<IDXGIAdapter4*>(Param_PonteiroNativo);
...
{
	//Adiciona uma referência ao ponteiro
	(reinterpret_cast<IDXGIAdapter4*>(PonteiroTrabalho))->AddRef();
}
...
void CarenDXGIAdapter4::LiberarReferencia()
{
	...
	ULONG RefCount = (reinterpret_cast<IDXGIAdapter4*>(PonteiroTrabalho))->Release();
...
/// </summary>
void CarenDXGIAdapter4::LimparDados()
{
...
CarenResult CarenDXGIAdapter4::ObterPaiObjeto(String^ Param_RIIDInterface, ICaren^ Param_Out_ObjetoPai)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
CarenResult CarenDXGIAdapter4::DefinirDadosPrivados(String^ Param_GuidIdentificao, UInt32 Param_TamanhoDados, ICaren^ Param_Dados)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
CarenResult CarenDXGIAdapter4::ObterDadosPrivados(String^ Param_GuidIdentificao, UInt32% Param_Ref_TamanhoDados, ICaren^ Param_Out_Dados)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
CarenResult CarenDXGIAdapter4::DefinirDadosPrivadosInterface(String^ Param_GuidInterface, ICaren^ Param_Interface)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
