...
/// <summary>
/// (Concluido - Fase de Testes) - Classe responsável por realizar consultas de informações da GPU.
/// </summary>
public ref class CarenD3D11Query :public ICarenD3D11Query
{
	/////////////////////////////////////////
...
public:
	static operator CarenD3D11Query^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD3D11Query^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD3D11Query();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID3D11Query*>(Param_Pointer.ToPointer());
...
public:
	...
	/// </summary>
	/// <param name="Param_Out_CarenD3D11Query">Recebe um ponteiro para a interface (Vazia).</param>
	static CarenResult CriarInstanciaVazia([Out] ICarenD3D11Query^% Param_Out_CarenD3D11Query)
	{
		//Variavel a ser retornada.
		CarenResult Resultado = CarenResult(E_FAIL, false);

		//Cria a interface
		Param_Out_CarenD3D11Query = gcnew CarenD3D11Query();
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD3D11Query::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
