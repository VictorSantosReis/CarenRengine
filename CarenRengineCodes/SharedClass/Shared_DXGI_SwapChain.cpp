...
//MÉTODOS DA CLASSE COMPARTILHADA (Shared_DXGISwapChain)
...
CarenResult Shared_DXGISwapChain::SetHDRMetaData(
	...
	ICaren^ Param_Metadata)
{
	...
	Hr = Param_MyPointerWork->SetHDRMetaData(MetadataType, Param_Size, PointerMetadata);
...
CarenResult Shared_DXGISwapChain::CheckColorSpaceSupport(IDXGISwapChain3* Param_MyPointerWork, CA_DXGI_COLOR_SPACE_TYPE Param_ColorSpace, [Out] CA_DXGI_SWAP_CHAIN_COLOR_SPACE_SUPPORT_FLAG% Param_Out_ColorSpaceSuporte)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CheckColorSpaceSupport(ColorSpaceCheck, &OutResultSuporte);
...
CarenResult Shared_DXGISwapChain::GetCurrentBackBufferIndex(IDXGISwapChain3* Param_MyPointerWork, [Out] UInt32% Param_Out_IndexBackBuffer)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);

	//Chama o método para realizar a operação.
	Param_Out_IndexBackBuffer = Param_MyPointerWork->GetCurrentBackBufferIndex();
...
CarenResult Shared_DXGISwapChain::ResizeBuffers1(
	...
	cli::array<ICaren^>^ Param_ArrayD3D12CommandQueue)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->ResizeBuffers1(Param_CountBuffer, Param_Largura, Param_Altura, DxgiFormato, SwapFlag, pArrayNodeMask, ppArrayD3D12Command);
...
CarenResult Shared_DXGISwapChain::SetColorSpace1(IDXGISwapChain3* Param_MyPointerWork, CA_DXGI_COLOR_SPACE_TYPE Param_ColorSpace)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->SetColorSpace1(static_cast<DXGI_COLOR_SPACE_TYPE>(Param_ColorSpace));
...
CarenResult Shared_DXGISwapChain::GetFrameLatencyWaitableObject(IDXGISwapChain2* Param_MyPointerWork, [Out] ICarenEvent^% Param_Out_HandleEvento)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	HandleEvento = Param_MyPointerWork->GetFrameLatencyWaitableObject();
...
CarenResult Shared_DXGISwapChain::GetMatrixTransform(IDXGISwapChain2* Param_MyPointerWork, [Out] CA_DXGI_MATRIX_3X2_F^% Param_Out_Matriz_32)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetMatrixTransform(&MatrixTransform);
...
CarenResult Shared_DXGISwapChain::GetMaximumFrameLatency(IDXGISwapChain2* Param_MyPointerWork, [Out] UInt32% Param_Out_LatenciaMaxima)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetMaximumFrameLatency(&OutLatencia);
...
CarenResult Shared_DXGISwapChain::GetSourceSize(IDXGISwapChain2* Param_MyPointerWork, [Out] UInt32% Param_Out_Largura, [Out] UInt32% Param_Out_Altura)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetSourceSize(&OutLargura, &OutAltura);
...
CarenResult Shared_DXGISwapChain::SetMatrixTransform(IDXGISwapChain2* Param_MyPointerWork, CA_DXGI_MATRIX_3X2_F^ Param_Matriz_32)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->SetMatrixTransform(pMatrixData);
...
CarenResult Shared_DXGISwapChain::SetMaximumFrameLatency(IDXGISwapChain2* Param_MyPointerWork, UInt32 Param_MaximoBackBufferLatencia)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->SetMaximumFrameLatency(Param_MaximoBackBufferLatencia);
...
CarenResult Shared_DXGISwapChain::SetSourceSize(IDXGISwapChain2* Param_MyPointerWork, UInt32 Param_Largura, UInt32 Param_Altura)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->SetSourceSize(Param_Largura, Param_Altura);
...
CarenResult Shared_DXGISwapChain::GetBackgroundColor(IDXGISwapChain1* Param_MyPointerWork, [Out] CA_DXGI_RGBA^% Param_Out_Cor)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetBackgroundColor(&dxgiRGBAColor);
...
CarenResult Shared_DXGISwapChain::GetCoreWindow(IDXGISwapChain1* Param_MyPointerWork, String^ Param_RIID, ICaren^ Param_Out_CoreWindow)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetCoreWindow(GuidCoreWin, &pCoreWin);
...
CarenResult Shared_DXGISwapChain::GetDesc1(IDXGISwapChain1* Param_MyPointerWork, [Out] CA_DXGI_SWAP_CHAIN_DESC1^% Param_Out_Desc)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetDesc1(&DescSwap);
...
CarenResult Shared_DXGISwapChain::GetFullscreenDesc(IDXGISwapChain1* Param_MyPointerWork, [Out] CA_DXGI_SWAP_CHAIN_FULLSCREEN_DESC^% Param_Out_DescFullScreenSwap)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetFullscreenDesc(&DescFullScreen);
...
CarenResult Shared_DXGISwapChain::GetHwnd(IDXGISwapChain1* Param_MyPointerWork, [Out] IntPtr% Param_Out_HWND)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetHwnd(&pHandle);
...
CarenResult Shared_DXGISwapChain::GetRestrictToOutput(IDXGISwapChain1* Param_MyPointerWork, [Out] ICarenDXGIOutput^% Param_Out_Saida)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetRestrictToOutput(&pSaida);
...
CarenResult Shared_DXGISwapChain::GetRotation(IDXGISwapChain1* Param_MyPointerWork, [Out] CA_DXGI_MODE_ROTATION% Param_Out_Rotacao)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetRotation(&Rotation);
...
CarenResult Shared_DXGISwapChain::IsTemporaryMonoSupported(IDXGISwapChain1* Param_MyPointerWork, [Out] Boolean% Param_Out_Suporte)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Suported = Param_MyPointerWork->IsTemporaryMonoSupported();
...
CarenResult Shared_DXGISwapChain::Present1(IDXGISwapChain1* Param_MyPointerWork, UInt32 Param_IntervaloSincronizacao, CA_DXGI_PRESENT Param_FlagsApresentacao, CA_DXGI_PRESENT_PARAMETERS^ Param_ParametrosApresentacao)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->Present1(Param_IntervaloSincronizacao, DxgiPresentFrameFlag, pParametrosApresentacao);
...
CarenResult Shared_DXGISwapChain::SetBackgroundColor(IDXGISwapChain1* Param_MyPointerWork, CA_DXGI_RGBA^ Param_Cor)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->SetBackgroundColor(pCorRGBA);
...
CarenResult Shared_DXGISwapChain::SetRotation(IDXGISwapChain1* Param_MyPointerWork, CA_DXGI_MODE_ROTATION Param_Rotacao)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->SetRotation(ModeRotation);
...
CarenResult Shared_DXGISwapChain::GetBuffer(IDXGISwapChain* Param_MyPointerWork, UInt32 Param_IndexBuffer, String^ Param_RiidInterface, ICaren^ Param_Out_InterfaceBuffer)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetBuffer(Param_IndexBuffer, RiidInterface, &pInterfaceReturn);
...
CarenResult Shared_DXGISwapChain::GetContainingOutput(IDXGISwapChain* Param_MyPointerWork, [Out] ICarenDXGIOutput^% Param_Out_MonitorSaida)

{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetContainingOutput(&pInterfaceReturn);
...
CarenResult Shared_DXGISwapChain::GetDesc(IDXGISwapChain* Param_MyPointerWork, [Out] Estruturas::CA_DXGI_SWAP_CHAIN_DESC^% Param_Out_DescricaoCadeiaSwap)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetDesc(&DescSwap);
...
CarenResult Shared_DXGISwapChain::GetFrameStatistics(IDXGISwapChain* Param_MyPointerWork, [Out] Estruturas::CA_DXGI_FRAME_STATISTICS^% Param_Out_FrameEstatisticas)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetFrameStatistics(&DescFrame);
...
CarenResult Shared_DXGISwapChain::GetFullscreenState(IDXGISwapChain* Param_MyPointerWork, [Out] Boolean% Param_Out_EstadoFullScreen, [Out] ICarenDXGIOutput^% Param_Out_MonitorSaida)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetFullscreenState(&StateFullScreen, &pInterfaceReturn);
...
CarenResult Shared_DXGISwapChain::GetLastPresentCount(IDXGISwapChain* Param_MyPointerWork, [Out] UInt32% Param_Out_QuantidadeChamadas)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetLastPresentCount(&CountPresentCalled);
...
CarenResult Shared_DXGISwapChain::Present(IDXGISwapChain* Param_MyPointerWork, UInt32 Param_IntervaloSincronizacao, Enumeracoes::CA_DXGI_PRESENT Param_Flags)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->Present(Param_IntervaloSincronizacao, FlagsPresent);
...
CarenResult Shared_DXGISwapChain::ResizeBuffers(IDXGISwapChain* Param_MyPointerWork, UInt32 Param_NumeroBuffers, UInt32 Param_Largura, UInt32 Param_Altura, Enumeracoes::CA_DXGI_FORMAT Param_NovoFormato, Enumeracoes::CA_DXGI_SWAP_CHAIN_FLAG Param_SwapChainFlags)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->ResizeBuffers(Param_NumeroBuffers, Param_Largura, Param_Altura, FormatoDXGI, FlagSwap);
...
CarenResult Shared_DXGISwapChain::ResizeTarget(IDXGISwapChain* Param_MyPointerWork, Estruturas::CA_DXGI_MODE_DESC^ Param_NovaDesc)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->ResizeTarget(pDesc);
...
CarenResult Shared_DXGISwapChain::SetFullscreenState(IDXGISwapChain* Param_MyPointerWork, Boolean Param_EstadoFullScreen, ICarenDXGIOutput^ Param_MonitorSaida)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->SetFullscreenState(StateFullScreen, pSaida ? pSaida : NULL);
