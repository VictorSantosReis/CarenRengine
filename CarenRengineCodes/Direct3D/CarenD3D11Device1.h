...
public:
	static operator CarenD3D11Device1^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD3D11Device1^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD3D11Device1();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID3D11Device1*>(Param_Pointer.ToPointer());
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD3D11Device1::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
