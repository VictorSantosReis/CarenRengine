...
public:
	static operator CarenD2D1Bitmap^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD2D1Bitmap^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD2D1Bitmap();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID2D1Bitmap*>(Param_Pointer.ToPointer());
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD2D1Bitmap::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
