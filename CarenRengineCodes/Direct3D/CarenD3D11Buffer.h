...
/// <summary>
/// (Concluido - Fase de Testes) - Classe responsável por acessar um recurso de buffer, que é a memória não estruturada. Buffers normalmente armazenam dados de vértice ou índice.
/// </summary>
public ref class CarenD3D11Buffer :public ICarenD3D11Buffer
{
	/////////////////////////////////////////
...
public:
	static operator CarenD3D11Buffer^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD3D11Buffer^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD3D11Buffer();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID3D11Buffer*>(Param_Pointer.ToPointer());
...
public:
	...
	/// </summary>
	/// <param name="Param_Out_CarenD3D11Buffer">Recebe um ponteiro para a interface (Vazia).</param>
	static CarenResult CriarInstanciaVazia([Out] ICarenD3D11Buffer^% Param_Out_CarenD3D11Buffer)
	{
		//Variavel a ser retornada.
		CarenResult Resultado = CarenResult(E_FAIL, false);

		//Cria a interface
		Param_Out_CarenD3D11Buffer = gcnew CarenD3D11Buffer();
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD3D11Buffer::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
...
public:
	...
	virtual CarenResult GetDesc([Out] Estruturas::CA_D3D11_BUFFER_DESC^% Param_Out_DescBuffer);
