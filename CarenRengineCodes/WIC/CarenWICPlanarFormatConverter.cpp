...
#include "../pch.h"
#include "CarenWICPlanarFormatConverter.h"

//Destruidor.
CarenWICPlanarFormatConverter::~CarenWICPlanarFormatConverter()
{
...
CarenResult CarenWICPlanarFormatConverter::ConsultarInterface(String^ Param_Guid, ICaren^ Param_InterfaceSolicitada)
{
...
CarenResult CarenWICPlanarFormatConverter::AdicionarPonteiro(IntPtr Param_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
	...
	PonteiroTrabalho = reinterpret_cast<IWICPlanarFormatConverter*>(Param_PonteiroNativo.ToPointer());
...
CarenResult CarenWICPlanarFormatConverter::AdicionarPonteiro(LPVOID Param_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
	...
	//Converte o ponteiro para o tipo especifico da classe.
	PonteiroTrabalho = reinterpret_cast<IWICPlanarFormatConverter*>(Param_PonteiroNativo);
...
CarenResult CarenWICPlanarFormatConverter::RecuperarPonteiro([Out] IntPtr% Param_Out_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
CarenResult CarenWICPlanarFormatConverter::RecuperarPonteiro(LPVOID* Param_Out_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
CarenResult CarenWICPlanarFormatConverter::RecuperarReferencias([Out] UInt64% Param_Out_Referencias)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
/// </summary>
CarenResult CarenWICPlanarFormatConverter::StatusPonteiro()
{
...
/// </summary>
Int32 CarenWICPlanarFormatConverter::ObterCodigoErro()
{
...
/// </summary>
void CarenWICPlanarFormatConverter::AdicionarReferencia()
{
	//Adiciona uma referência ao ponteiro
...
/// </summary>
void CarenWICPlanarFormatConverter::LiberarReferencia()
{
...
/// </summary>
void CarenWICPlanarFormatConverter::LimparDados()
{
...
/// </summary>
void CarenWICPlanarFormatConverter::Finalizar()
{
	//////////////////////
	//Código de descarte//
	//////////////////////
	...
	this->~CarenWICPlanarFormatConverter();
}



// Métodos da interface proprietária(ICarenWICPlanarFormatConverter)
...
CarenResult CarenWICPlanarFormatConverter::CanConvert(
...
[Out] Boolean% Param_Out_Conversao)
{
	...
	//Variaveis a serem utilizadas.
	Utilidades Util;
	GUID* vi_ArrayGuidFormatos = CriarMatrizEstruturas<GUID>(Param_QuantidadeFormatos);
	GUID vi_GuidDestino = GUID_NULL;
	BOOL vi_OutConversao = FALSE;

	//Abre um for para criar os guids e definir na matriz.
	for (UINT32 i = 0; i < Param_QuantidadeFormatos; i++)
	{
		//Converte e define no index especificado.
		vi_ArrayGuidFormatos[i] = Util.CreateGuidFromString(Param_ArrayGuidFormatosPixel[i]);
	}

	//Converte a string para o guid de destino.
	vi_GuidDestino = Util.CreateGuidFromString(Param_GuidPixelDestino);

	//Chama o método para realizar a operação.
	Hr = PonteiroTrabalho->CanConvert(vi_ArrayGuidFormatos, Param_QuantidadeFormatos, vi_GuidDestino, &vi_OutConversao);
	...
	Param_Out_Conversao = vi_OutConversao ? true : false;

Done:;
	//Libera a memória utilizada pela matriz de guids.
...
CarenResult CarenWICPlanarFormatConverter::Initialize(
...
CA_WICBitmapPaletteType Param_PaletaConversao)
{
	...
	//Variaveis a serem utilizadas.
	Utilidades Util;
	IWICBitmapSource** vi_ArrayPlanos = CriarMatrizPonteiros<IWICBitmapSource>(Param_QuantidadeImagens);
	GUID vi_GuidDestino = GUID_NULL;
	IWICPalette* vi_Paleta = Nulo;

	//Define os ponteiros na matriz de array de planos
	Util.CopiarPonteirosGerenciado_ToNativo(vi_ArrayPlanos, Param_ArrayPlanes, Param_QuantidadeImagens);

	//Converte a string para o guid de destino.
	vi_GuidDestino = Util.CreateGuidFromString(Param_GuidFormatoPixelDestino);

	//Recupera o ponteiro para a peleta usada para conversão.
	CarenGetPointerFromICarenSafe(Param_Paleta, vi_Paleta);

	//Chama o método para realizar a operação.
	Hr = PonteiroTrabalho->Initialize(
		vi_ArrayPlanos,
		Param_QuantidadeImagens,
		vi_GuidDestino,
		static_cast<WICBitmapDitherType>(Param_Dither),
		vi_Paleta,
		Param_PorcentagemAlfa,
		static_cast<WICBitmapPaletteType>(Param_PaletaConversao)
	);
...
Done:;
	...
	DeletarMatrizPonteirosSafe(&vi_ArrayPlanos);
...
CarenResult CarenWICPlanarFormatConverter::CopyPalette(ICaren^ Param_Palette)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
CarenResult CarenWICPlanarFormatConverter::CopyPixels(
...
CarenResult CarenWICPlanarFormatConverter::GetPixelFormat([Out] String^% Param_Out_FormatoPixel)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
CarenResult CarenWICPlanarFormatConverter::GetResolution(
...
CarenResult CarenWICPlanarFormatConverter::GetSize(
