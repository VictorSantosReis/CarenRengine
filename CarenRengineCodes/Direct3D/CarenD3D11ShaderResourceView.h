...
/// <summary>
/// (Concluido - Fase de Testes) - Classe responsável por representar uma interface de visões de recursos obscuros que especifica os subrecursos que um Shader pode acessar durante a renderização.
/// </summary>
public ref class CarenD3D11ShaderResourceView : public ICarenD3D11ShaderResourceView
{
	/////////////////////////////////////////
...
public:
	static operator CarenD3D11ShaderResourceView^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD3D11ShaderResourceView^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD3D11ShaderResourceView();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID3D11ShaderResourceView*>(Param_Pointer.ToPointer());
...
public:
	...
	/// </summary>
	/// <param name="Param_Out_CarenD3D11ShaderResourceView">Recebe um ponteiro para a interface (Vazia).</param>
	static CarenResult CriarInstanciaVazia([Out] ICarenD3D11ShaderResourceView^% Param_Out_CarenD3D11ShaderResourceView)
	{
		//Variavel a ser retornada.
		CarenResult Resultado = CarenResult(E_FAIL, false);

		//Cria a interface
		Param_Out_CarenD3D11ShaderResourceView = gcnew CarenD3D11ShaderResourceView();
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD3D11ShaderResourceView::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
...
public:
	...
	virtual CarenResult GetDesc([Out] Estruturas::CA_D3D11_SHADER_RESOURCE_VIEW_DESC^% Param_Out_DescSombreadorRecurso);
