...
public:
	static operator CarenDXGIAdapter4^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenDXGIAdapter4^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenDXGIAdapter4();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<IDXGIAdapter4*>(Param_Pointer.ToPointer());
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenDXGIAdapter4::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
...
public:
	...
	virtual CarenResult GetDesc3([Out] CA_DXGI_ADAPTER_DESC3^% Param_Out_DescAdaptador);
...
public:
	...
	virtual CarenResult RegisterHardwareContentProtectionTeardownStatusEvent(ICarenWindowsEvent^ Param_Evento, [Out] UInt32 Param_Out_Cookie);
	...
	virtual CarenResult RegisterVideoMemoryBudgetChangeNotificationEvent(ICarenWindowsEvent^ Param_Evento, [Out] UInt32 Param_Out_Cookie);
