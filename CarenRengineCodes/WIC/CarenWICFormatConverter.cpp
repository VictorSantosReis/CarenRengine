...
CarenResult CarenWICFormatConverter::ConsultarInterface(String^ Param_Guid, ICaren^ Param_InterfaceSolicitada)
{
...
CarenResult CarenWICFormatConverter::AdicionarPonteiro(IntPtr Param_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
	...
	PonteiroTrabalho = reinterpret_cast<IWICFormatConverter*>(Param_PonteiroNativo.ToPointer());
...
CarenResult CarenWICFormatConverter::AdicionarPonteiro(LPVOID Param_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
	...
	//Converte o ponteiro para o tipo especifico da classe.
	PonteiroTrabalho = reinterpret_cast<IWICFormatConverter*>(Param_PonteiroNativo);
...
CarenResult CarenWICFormatConverter::RecuperarPonteiro([Out] IntPtr% Param_Out_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
CarenResult CarenWICFormatConverter::RecuperarPonteiro(LPVOID* Param_Out_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
CarenResult CarenWICFormatConverter::RecuperarReferencias([Out] UInt64% Param_Out_Referencias)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
/// </summary>
CarenResult CarenWICFormatConverter::StatusPonteiro()
{
...
/// </summary>
Int32 CarenWICFormatConverter::ObterCodigoErro()
{
...
/// </summary>
void CarenWICFormatConverter::AdicionarReferencia()
{
	//Adiciona uma referência ao ponteiro
...
/// </summary>
void CarenWICFormatConverter::LiberarReferencia()
{
...
/// </summary>
void CarenWICFormatConverter::LimparDados()
{
...
/// </summary>
void CarenWICFormatConverter::Finalizar()
{
	//////////////////////
	//Código de descarte//
	//////////////////////
	...
	this->~CarenWICFormatConverter();
}



// Métodos da interface proprietária(ICarenWICFormatConverter)
...
CarenResult CarenWICFormatConverter::CanConvert(
...
[Out] Boolean% Param_Out_Conversao)
{
	...
	//Variaveis a serem utilizadas.
	Utilidades Util;
	GUID vi_GuidPixelOrigem = GUID_NULL;
	GUID vi_GuidPixelDestino = GUID_NULL;
	BOOL vi_OutConversao = FALSE;

	//Converte as strings para Guid.
	vi_GuidPixelOrigem = Util.CreateGuidFromString(Param_GuidFormatoPixelOrigem);
	vi_GuidPixelDestino = Util.CreateGuidFromString(Param_GuidFormatoPixelDestino);

	//Chama o método para realizar a operação.
	Hr = PonteiroTrabalho->CanConvert(vi_GuidPixelOrigem, vi_GuidPixelDestino, &vi_OutConversao);
;
	//Processa o resultado da chamada.
	Resultado.ProcessarCodigoOperacao(Hr);
...
CarenResult CarenWICFormatConverter::Initialize(
...
CA_WICBitmapPaletteType Param_TipoPaletaConversao)
{
	...
	//Variaveis a serem utilizadas.
	Utilidades Util;
	IWICBitmapSource* vi_pBitmapFonte = Nulo;
	GUID vi_GuidPixel = GUID_NULL;
	WICBitmapDitherType vi_ditherType = static_cast<WICBitmapDitherType>(Param_Dither);
	IWICPalette* vi_pPaleta = Nulo;
	WICBitmapPaletteType vi_TipoPaletaConversao = static_cast<WICBitmapPaletteType>(Param_TipoPaletaConversao);

	//Recupera o ponteiro para a fonte do bitmap.
	Resultado = RecuperarPonteiroCaren(Param_BitmapFonte, &vi_pBitmapFonte);

	//Sai do método em caso de erro.
	SairOnError(Resultado);

	//Converte a string para Guid
	vi_GuidPixel = Util.CreateGuidFromString(Param_GuidFormatoPixelDestino);

	//Recupera o ponteiro para a peleta de cores.
	Resultado = RecuperarPonteiroCaren(Param_Paleta, &vi_pPaleta);

	//Sai do método em caso de erro.
	SairOnError(Resultado);

	//Chama o método para realizar a operação.
	Hr = PonteiroTrabalho->Initialize(vi_pBitmapFonte, vi_GuidPixel, vi_ditherType, vi_pPaleta, Param_PorcentagemAlfa, vi_TipoPaletaConversao);
...
CarenResult CarenWICFormatConverter::CopyPalette(ICaren^ Param_Palette)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
CarenResult CarenWICFormatConverter::CopyPixels(
...
CarenResult CarenWICFormatConverter::GetPixelFormat([Out] String^% Param_Out_FormatoPixel)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
CarenResult CarenWICFormatConverter::GetResolution(
...
CarenResult CarenWICFormatConverter::GetSize(
