...
/// <summary>
/// (Concluido - Fase de Testes) - Classe responsável por representar um predicado que determina se a geometria deve ser processada dependendo dos resultados de uma chamada de sorteio anterior.
/// </summary>
public ref class CarenD3D11Predicate : public ICarenD3D11Predicate
{
	/////////////////////////////////////////
...
public:
	static operator CarenD3D11Predicate^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD3D11Predicate^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD3D11Predicate();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID3D11Predicate*>(Param_Pointer.ToPointer());
...
public:
	...
	/// </summary>
	/// <param name="Param_Out_CarenD3D11Predicate">Recebe um ponteiro para a interface (Vazia).</param>
	static CarenResult CriarInstanciaVazia([Out] ICarenD3D11Predicate^% Param_Out_CarenD3D11Predicate)
	{
		//Variavel a ser retornada.
		CarenResult Resultado = CarenResult(E_FAIL, false);

		//Cria a interface
		Param_Out_CarenD3D11Predicate = gcnew CarenD3D11Predicate();
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD3D11Predicate::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
...
public:
	//ESSA INTERFACE NAO POSSUI MÉTODOS PROPRIOS.
 ...
 public:
	 ...
	 virtual CarenResult GetDesc([Out] Estruturas::CA_D3D11_QUERY_DESC^% Param_Out_DescConsulta);
