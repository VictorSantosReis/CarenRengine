...
/// <summary>
/// (Concluido - Fase de Testes) - Classe responsável por representar uma interface de estado de mesclagem que guarda a descrição do estado para o estagio de fusão de saida(MergeState).
/// </summary>
public ref class CarenD3D11BlendState :public ICarenD3D11BlendState
{
	/////////////////////////////////////////
...
public:
	static operator CarenD3D11BlendState^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD3D11BlendState^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD3D11BlendState();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID3D11BlendState*>(Param_Pointer.ToPointer());
...
public:
	...
	/// </summary>
	/// <param name="Param_Out_CarenD3D11BlendState">Recebe um ponteiro para a interface (Vazia).</param>
	static CarenResult CriarInstanciaVazia([Out] ICarenD3D11BlendState^% Param_Out_CarenD3D11BlendState)
	{
		//Variavel a ser retornada.
		CarenResult Resultado = CarenResult(E_FAIL, false);

		//Cria a interface
		Param_Out_CarenD3D11BlendState = gcnew CarenD3D11BlendState();
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD3D11BlendState::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
...
public:
	...
	virtual CarenResult GetDesc([Out] Estruturas::CA_D3D11_BLEND_DESC^% Param_Out_DescEstado);
