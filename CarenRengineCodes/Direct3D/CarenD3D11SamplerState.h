...
/// <summary>
/// (Concluido - Fase de Testes) - Classe responsável por conter uma descrição para o estado (SAMPLER) que você pode liga a qualquer fase mais o sombreada do gasoduto para referência para operações de amostra de textura.
/// </summary>
public ref class CarenD3D11SamplerState :public ICarenD3D11SamplerState
{
	/////////////////////////////////////////
...
public:
	static operator CarenD3D11SamplerState^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD3D11SamplerState^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD3D11SamplerState();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID3D11SamplerState*>(Param_Pointer.ToPointer());
...
public:
	...
	/// </summary>
	/// <param name="Param_Out_CarenD3D11SamplerState">Recebe um ponteiro para a interface (Vazia).</param>
	static CarenResult CriarInstanciaVazia([Out] ICarenD3D11SamplerState^% Param_Out_CarenD3D11SamplerState)
	{
		//Variavel a ser retornada.
		CarenResult Resultado = CarenResult(E_FAIL, false);

		//Cria a interface
		Param_Out_CarenD3D11SamplerState = gcnew CarenD3D11SamplerState();
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD3D11SamplerState::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
...
public:
	...
	virtual CarenResult GetDesc([Out] Estruturas::CA_D3D11_SAMPLER_DESC^% Param_Out_DescAmostrador);
