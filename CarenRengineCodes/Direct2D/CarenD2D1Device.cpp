...
//Destruidor.
CarenD2D1Device::~CarenD2D1Device()
{
...
CarenD2D1Device::CarenD2D1Device(ICarenDXGIDevice^ Param_DXGIDevice, CA_D2D1_CREATION_PROPERTIES^ Param_PropsCreate)
{
	...
	Utilidades Util;
	IDXGIDevice* vi_pDXGIDevice = Nulo;
	D2D1_CREATION_PROPERTIES* vi_pPropsCreate = Nulo;
	ID2D1Device* vi_pOutD2D1Device = Nulo;

	//Verfifica se a interface do dispositivo DXGI é valida.
	if (!ObjetoGerenciadoValido(Param_DXGIDevice))
		throw gcnew NullReferenceException("A interface no parametro (Param_DXGIDevice) não pode ser NULA!");

	//Tenta recuperar o ponteiro para a interface.
	Resultado = RecuperarPonteiroCaren(Param_DXGIDevice, &vi_pDXGIDevice);

	//Verifica se não houve algum erro
	if (!CarenSucesso(Resultado))
		throw gcnew Exception("Falhou ao tentar recuperar o ponteiro para a interface do dispositivo DXGI.");
	...
	Hr = D2D1CreateDevice(vi_pDXGIDevice, const_cast<D2D1_CREATION_PROPERTIES*>(vi_pPropsCreate), &vi_pOutD2D1Device);
	...
	PonteiroTrabalho = vi_pOutD2D1Device;
...
CarenResult CarenD2D1Device::ConsultarInterface(String^ Param_Guid, ICaren^ Param_InterfaceSolicitada)
{
...
CarenResult CarenD2D1Device::AdicionarPonteiro(IntPtr Param_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
	...
	PonteiroTrabalho = reinterpret_cast<ID2D1Device*>(Param_PonteiroNativo.ToPointer());
...
CarenResult CarenD2D1Device::AdicionarPonteiro(LPVOID Param_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
	...
	//Converte o ponteiro para o tipo especifico da classe.
	PonteiroTrabalho = reinterpret_cast<ID2D1Device*>(Param_PonteiroNativo);
...
CarenResult CarenD2D1Device::RecuperarPonteiro([Out] IntPtr% Param_Out_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
CarenResult CarenD2D1Device::RecuperarPonteiro(LPVOID* Param_Out_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
CarenResult CarenD2D1Device::RecuperarReferencias([Out] UInt64% Param_Out_Referencias)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
/// </summary>
CarenResult CarenD2D1Device::StatusPonteiro()
{
...
/// </summary>
Int32 CarenD2D1Device::ObterCodigoErro()
{
...
/// </summary>
void CarenD2D1Device::AdicionarReferencia()
{
	//Adiciona uma referência ao ponteiro
...
/// </summary>
void CarenD2D1Device::LiberarReferencia()
{
...
/// </summary>
void CarenD2D1Device::LimparDados()
{
...
/// </summary>
void CarenD2D1Device::Finalizar()
{
	//////////////////////
	//Código de descarte//
	//////////////////////
	...
	this->~CarenD2D1Device();
}



// Métodos da interface proprietária(ICarenD2D1Device)
...
void CarenD2D1Device::ClearResources(UInt32 Param_Miliseconds)
{
...
CarenResult CarenD2D1Device::CreateDeviceContext(
...
CarenResult CarenD2D1Device::CreatePrintControl(
...
void CarenD2D1Device::GetMaximumTextureMemory([Out] UInt64% Param_Out_MaxSizeMemory)
{
	//Chama o método para realizar a operação.
...
void CarenD2D1Device::SetMaximumTextureMemory(UInt64 Param_MaxSizeMemory)
{
	//Chama o método para realizar a operação.
...
void CarenD2D1Device::GetFactory(ICaren^ Param_Out_Factory)
{
	//Variaveis a serem utilizadas.
