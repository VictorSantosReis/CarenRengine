...
//Destruidor.
CarenD3D11ClassInstance::~CarenD3D11ClassInstance()
{
...
CarenResult CarenD3D11ClassInstance::ConsultarInterface(String^ Param_Guid, ICaren^ Param_InterfaceSolicitada)
{
...
CarenResult CarenD3D11ClassInstance::AdicionarPonteiro(IntPtr Param_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	PonteiroTrabalho = reinterpret_cast<ID3D11ClassInstance*>(Param_PonteiroNativo.ToPointer());
...
CarenResult CarenD3D11ClassInstance::AdicionarPonteiro(LPVOID Param_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Converte o ponteiro para o tipo especifico da classe.
	PonteiroTrabalho = reinterpret_cast<ID3D11ClassInstance*>(Param_PonteiroNativo);
...
CarenResult CarenD3D11ClassInstance::RecuperarPonteiro([Out] IntPtr% Param_Out_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
CarenResult CarenD3D11ClassInstance::RecuperarPonteiro(LPVOID* Param_Out_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
CarenResult CarenD3D11ClassInstance::RecuperarReferencias([Out] UInt64% Param_Out_Referencias)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
/// </summary>
CarenResult CarenD3D11ClassInstance::StatusPonteiro()
{
...
/// </summary>
Int32 CarenD3D11ClassInstance::ObterCodigoErro()
{
...
/// </summary>
void CarenD3D11ClassInstance::AdicionarReferencia()
{
	//Adiciona uma referência ao ponteiro
...
/// </summary>
void CarenD3D11ClassInstance::LiberarReferencia()
{
...
/// </summary>
void CarenD3D11ClassInstance::LimparDados()
{
...
/// </summary>
void CarenD3D11ClassInstance::Finalizar()
{
	//////////////////////
	//Código de descarte//
	//////////////////////
...
CarenResult CarenD3D11ClassInstance::ObterClassLinkage([Out] ICarenD3D11ClassLinkage^% Param_Out_ClassLinkageAssociado) 
{
	...
	//Variaveis a serem utilizadas.
	ID3D11ClassLinkage* pClasseLigacao = NULL;

	//Chama o método para realizar a operação
	PonteiroTrabalho->GetClassLinkage(&pClasseLigacao);

	//Verifica se a interface é valida
	if (ObjetoValido(pClasseLigacao))
	{
	...
	Param_Out_ClassLinkageAssociado = gcnew CarenD3D11ClassLinkage();

	//Define o ponteiro de trabalho
	Param_Out_ClassLinkageAssociado->AdicionarPonteiro(pClasseLigacao);
...
CarenResult CarenD3D11ClassInstance::GetDesc([Out] Estruturas::CA_D3D11_CLASS_INSTANCE_DESC^% Param_Out_DescClasseHLSL) 
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Variaveis a serem utilizadas.
	Utilidades Util;
	D3D11_CLASS_INSTANCE_DESC DescInstance;

	//Chama o método para realizar a operação
	PonteiroTrabalho->GetDesc(&DescInstance);

	//Converter a estrutura e define no parametro de saida.
	Param_Out_DescClasseHLSL = Util.ConverterD3D11_CLASS_INSTANCE_DESCUnManaged_ToManaged(&DescInstance);
...
CarenResult CarenD3D11ClassInstance::ObterNomeInstancia([Out] String^% Param_Out_NomeInstancia) 
{
	...
	//Variaveis a serem utilizadas.
	Utilidades Util;
	LPSTR pNomeInstancia = NULL;
	SIZE_T LarguraBuffer = 0;

	//Chama o método para realizar a operação
	PonteiroTrabalho->GetInstanceName(pNomeInstancia, &LarguraBuffer);

	//Verifica se obteve sucesso.
	if (ObjetoValido(pNomeInstancia))
	{
		//Deixa o método continuar.
	}
	else
	...
	//Converte e define o nome da classe no parametro de saida
	Param_Out_NomeInstancia = gcnew String(pNomeInstancia);
...
CarenResult CarenD3D11ClassInstance::ObterNomeTipoHLSL([Out] String^% Param_Out_NomeTipoHLSL)
{
	...
	//Variaveis a serem utilizadas.
	Utilidades Util;
	LPSTR pNomeTipo = NULL;
	SIZE_T LarguraBuffer = 0;

	//Chama o método para realizar a função
	PonteiroTrabalho->GetTypeName(pNomeTipo, &LarguraBuffer);

	//Verifica se obteve sucesso.
	if (ObjetoValido(pNomeTipo))
	{
		//Deixa o método continuar.
	}
	else
	...
	//Converte e define o nome do tipo da classe HLSL.
	Param_Out_NomeTipoHLSL = gcnew String(pNomeTipo);
