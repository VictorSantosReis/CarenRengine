...
public:
	static operator CarenWICBitmapFrameEncode^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenWICBitmapFrameEncode^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenWICBitmapFrameEncode();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<IWICBitmapFrameEncode*>(Param_Pointer.ToPointer());
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenWICBitmapFrameEncode::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
