...
/// <summary>
/// (Concluido - Fase de Testes) - Classe responsável por acessar dados de Textura 2D, que são de memória estruturada.
/// </summary>
public ref class CarenD3D11Texture2D1 : public ICarenD3D11Texture2D1
{
	/////////////////////////////////////////
	//Objeto gerenciado por essa interface.//
	/////////////////////////////////////////

	//Ponteiro para a interface (ID3D11Texture2D1).
	ID3D11Texture2D1* PonteiroTrabalho = NULL;
...
public:
	static operator CarenD3D11Texture2D1^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD3D11Texture2D1^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD3D11Texture2D1();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID3D11Texture2D1*>(Param_Pointer.ToPointer());
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD3D11Texture2D1::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
	...
	//Métodos da interface(ICarenD3D11Texture2D1)
public:
	/// <summary>
	...
	virtual CarenResult GetDesc1([Out] Estruturas::CA_D3D11_TEXTURE2D_DESC1^% Param_Out_PropsTextura2D);
...
public:
	...
	/// </summary>
	/// <param name="Param_Out_PropsTextura2D">Retorna uma estrutura com as descrições do recuso.</param>
	virtual CarenResult GetDesc([Out] Estruturas::CA_D3D11_TEXTURE2D_DESC^% Param_Out_PropsTextura2D);
