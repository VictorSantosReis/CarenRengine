...
//MÉTODOS DA CLASSE COMPARTILHADA (Shared_D3D11Device)
...
CarenResult Shared_D3D11Device::CreateFence(
	...
	ICarenD3D11Fence^% Param_Ref_FenceInterface)
{
	...
	Hr = Param_MyPointerWork->CreateFence(Param_DadosInicias, FenceFlag, RiidInterface, &pOutFence);
...
CarenResult Shared_D3D11Device::OpenSharedFence(
	ID3D11Device5* Param_MyPointerWork,
	IntPtr Param_Handle,
	String^ Param_RIIDInterface,
	ICarenD3D11Fence^% Param_Ref_FenceInterface)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->OpenSharedFence(pSharedHandle, RiidInterface, &pOutFence);
...
CarenResult Shared_D3D11Device::RegisterDeviceRemovedEvent(
	...
	[Out] UInt32 Param_Out_Cookie)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->RegisterDeviceRemovedEvent(pHandleEvento, &OutCookieEvent);
...
CarenResult Shared_D3D11Device::UnregisterDeviceRemoved(
	ID3D11Device4* Param_MyPointerWork, 
	UInt32 Param_Cookie)
{
	//Variavel a ser retornada.
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->UnregisterDeviceRemoved(static_cast<DWORD>(Param_Cookie));
...
CarenResult Shared_D3D11Device::CreateDeferredContext3(
	...
	[Out] ICarenD3D11DeviceContext3^% Param_Out_ContextoDiferido)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateDeferredContext3(Param_Flags, &pOutContextoDispositivo3);
...
CarenResult Shared_D3D11Device::CreateQuery1(
	ID3D11Device3* Param_MyPointerWork,
	...
	ICarenD3D11Query1^% Param_Ref_Query)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateQuery1(pDesc, ObjetoGerenciadoValido(Param_Ref_Query) ? &pInterfaceReturn : NULL);
...
CarenResult Shared_D3D11Device::CreateRasterizerState2(
	ID3D11Device3* Param_MyPointerWork,
	...
	ICarenD3D11RasterizerState2^% Param_Ref_Rasterizador)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateRasterizerState2(pDescRasterizer, ObjetoGerenciadoValido(Param_Ref_Rasterizador) ? &pOutRasterizador : NULL);
...
CarenResult Shared_D3D11Device::CreateRenderTargetView1(
	ID3D11Device3* Param_MyPointerWork,
	...
	ICarenD3D11RenderTargetView1^% Param_Ref_RenderTarget)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateRenderTargetView1(pRecurso, pDescRenderTarget, ObjetoGerenciadoValido(Param_Ref_RenderTarget) ? &pOutRenderTarget : NULL);
...
/// parâmetro como NULO para validar os outros parâmetros de entrada (o método retornará SS_FALSE se os outros parâmetros de entrada passarem pela validação).</param>
CarenResult Shared_D3D11Device::CreateShaderResourceView1(
	ID3D11Device3* Param_MyPointerWork,
	ICarenD3D11Resource^ Param_Recurso,
	...
	ICarenD3D11ShaderResourceView1^% Param_Ref_Shader)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateShaderResourceView1(pRecurso, pDescShaderView, ObjetoGerenciadoValido(Param_Ref_Shader) ? &pOutSharedView : NULL);
...
/// parâmetros de entrada (o método retornará SS_FALSE se os outros parâmetros de entrada passarem pela validação).</param>
CarenResult Shared_D3D11Device::CreateTexture2D1(
	ID3D11Device3* Param_MyPointerWork,
	...
	ICarenD3D11Texture2D1^% Param_Ref_Textura2D)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateTexture2D1(pDesc, pArraySubresourceData, ObjetoGerenciadoValido(Param_Ref_Textura2D) ? &pInterfaceReturn : NULL);
...
/// parâmetros de entrada (o método retornará SS_FALSE se os outros parâmetros de entrada passarem pela validação).</param>
CarenResult Shared_D3D11Device::CreateTexture3D1(
	ID3D11Device3* Param_MyPointerWork,
	...
	ICarenD3D11Texture3D1^% Param_Ref_Textura3D)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateTexture3D1(pDesc, pArraySubresourceData, ObjetoGerenciadoValido(Param_Ref_Textura3D) ? &pInterfaceReturn : NULL);
...
/// parâmetro como NULO para validar os outros parâmetros de entrada (o método retornará SS_FALSE se os outros parâmetros de entrada passarem pela validação).</param>
CarenResult Shared_D3D11Device::CreateUnorderedAccessView1(
	ID3D11Device3* Param_MyPointerWork,
	ICarenD3D11Resource^ Param_Recurso,
	...
	ICarenD3D11UnorderedAccessView1^% Param_Ref_ViewUnordered)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateUnorderedAccessView1(pRecurso, pDescUnorderedView, ObjetoGerenciadoValido(Param_Ref_ViewUnordered) ? &pOutUnorderedView : NULL);
...
CarenResult Shared_D3D11Device::GetImmediateContext3(
	ID3D11Device3* Param_MyPointerWork, 
	[Out] ICarenD3D11DeviceContext3^% Param_Out_Contexto)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GetImmediateContext3(&pOutContextoDispositivo3);
...
CarenResult Shared_D3D11Device::ReadFromSubresource(
	ID3D11Device3* Param_MyPointerWork,
	...
	CA_D3D11_BOX^ Param_CaixaOrigem)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->ReadFromSubresource(
		pBufferDestino,
...
CarenResult Shared_D3D11Device::WriteToSubresource(
	ID3D11Device3* Param_MyPointerWork,
	...
	UInt32 Param_OrigemPassoProfundidade)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->WriteToSubresource(
		pRecursoDestino,
...
CarenResult Shared_D3D11Device::CheckMultisampleQualityLevels1(
	...
	[Out] UInt32% Param_Out_NiveisQualidade)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CheckMultisampleQualityLevels1(dxgiFormato, Param_QuantidadeSample, FlagsMetodo, &OutNiveisQualidadeDisponiveis);
...
CarenResult Shared_D3D11Device::CreateDeferredContext2(
	ID3D11Device2* Param_MyPointerWork,
	...
	[Out] ICarenD3D11DeviceContext2^% Param_Out_ContextoDiferido)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateDeferredContext2(Param_ContextFlags, &pOutContextoDispositivo2);
...
CarenResult Shared_D3D11Device::GetImmediateContext2(
	ID3D11Device2* Param_MyPointerWork, 
	[Out] ICarenD3D11DeviceContext2^% Param_Out_Contexto)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GetImmediateContext2(&pOutContextoDispositivo2);
...
CarenResult Shared_D3D11Device::GetResourceTiling(
	ID3D11Device2* Param_MyPointerWork,
	ICarenD3D11Resource^ Param_Recurso,
	...
	[Out] CA_D3D11_SUBRESOURCE_TILING^% Param_Out_DescTileSubrecurso)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GetResourceTiling(
		pRecurso,
		&OutNumeroTiles,
		&OutPackedDesc,
		&OutTileShape,
...
CarenResult Shared_D3D11Device::CreateBlendState1(
	...
	[Out] ICarenD3D11BlendState1^% Param_Out_BlendState)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateBlendState1(&OutDesc, &pBlendState1);
...
CarenResult Shared_D3D11Device::CreateDeferredContext1(
	ID3D11Device1* Param_MyPointerWork, 
	UInt32 Param_ContextFlags, 
	[Out] ICarenD3D11DeviceContext1^% Param_Out_ContextoDiferido)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateDeferredContext1(Param_ContextFlags, &pContextoDiferido1);
...
CarenResult Shared_D3D11Device::CreateDeviceContextState(
	ID3D11Device1* Param_MyPointerWork,
	...
	[Out] ICarenD3DDeviceContextState^% Param_Out_EstadoContextoDispositivo)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateDeviceContextState(StateFlag, pArrayFeatures, Param_QuantidadeNiveisRecurso, Param_VersaoSDK, IdInterface, &OutNivelRecursoDefinido, &pDispositivoEstado);
...
CarenResult Shared_D3D11Device::CreateRasterizerState1(
	ID3D11Device1* Param_MyPointerWork,
	...
	[Out] ICarenD3D11RasterizerState1^% Param_Out_Rasterizador)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateRasterizerState1(&OutDescRasterizer, &pOutRasterizador);
...
CarenResult Shared_D3D11Device::GetImmediateContext1(
	ID3D11Device1* Param_MyPointerWork, 
	[Out] ICarenD3D11DeviceContext1^% Param_Out_ContextoImediato)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GetImmediateContext1(&pOutContextoImdt1);
...
CarenResult Shared_D3D11Device::OpenSharedResource1(
	ID3D11Device1* Param_MyPointerWork,
	...
	ICaren^% Param_Ref_InterfaceRecurso)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->OpenSharedResource1(pHandleRecurso, RiidInterface, &pInterfaceRecurso);
...
/// <param name="Param_Ref_InterfaceRecurso">Retorna um ponteiro para a interface de recurso requisitada no (Param_RIIDInterface). Este parametro 
/// não pode ser NULO.</param>
CarenResult Shared_D3D11Device::OpenSharedResourceByName(
	ID3D11Device1* Param_MyPointerWork,
	String^ Param_Nome,
	...
	ICaren^% Param_Ref_InterfaceRecurso)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->OpenSharedResourceByName(pNomeRecurso, Param_TipoAcesso, RiidInterface, &pInterfaceRecurso);
...
CarenResult Shared_D3D11Device::CheckCounter(
...
)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CheckCounter(pDesc, &TipoContador, &pNumeroContadores, pNomeContador, &pLarguraNomeContador, pNomeUnidadeMedidas, &pLarguraUnidadeMedidas, pDescricaoContador, &pLarguraDescricaoContador);
...
CarenResult Shared_D3D11Device::CheckCounterInfo(
	ID3D11Device* Param_MyPointerWork, 
	[Out] Estruturas::CA_D3D11_INFO_CONTADOR^% Param_Out_InfoContador)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->CheckCounterInfo(&pCounterInfo);
...
CarenResult Shared_D3D11Device::CheckFeatureSupport(
	ID3D11Device* Param_MyPointerWork,
...
)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CheckFeatureSupport(FeatureEnum, &FeatureData, sizeof(D3D11_FEATURE_DATA_THREADING));
...
CarenResult Shared_D3D11Device::CheckFormatSupport(
	ID3D11Device* Param_MyPointerWork,
	...
	[Out] Enumeracoes::CA_D3D11_FORMAT_SUPPORT% Param_Out_RecursosSuportados)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CheckFormatSupport(Formato, &pFormatoD3D11Suportado);
...
CarenResult Shared_D3D11Device::CheckMultisampleQualityLevels(
	ID3D11Device* Param_MyPointerWork,
	Enumeracoes::CA_DXGI_FORMAT Param_Formato,
	...
	[Out] UInt32% Param_Out_NumeroNiveisQualidade)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CheckMultisampleQualityLevels(Formato, Param_NumeroAmostras, &QuantidadeNiveisQualidade);
...
CarenResult Shared_D3D11Device::CreateBlendState(
	ID3D11Device* Param_MyPointerWork,
	...
	[Out] ICarenD3D11BlendState^% Param_Out_BlendState)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateBlendState(pBlendDesc, &pBlendState);
...
CarenResult Shared_D3D11Device::CreateBuffer(
	ID3D11Device* Param_MyPointerWork,
	...
	[Out] ICarenD3D11Buffer^% Param_Out_InterfaceBuffer)
{
	...
	if (Param_ValidarPametros)
	{
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateBuffer(pBufferDesc, ObjetoValido(pSubresourceDesc) ? pSubresourceDesc : NULL, NULL);
	}
	else
	{
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateBuffer(pBufferDesc, ObjetoValido(pSubresourceDesc) ? pSubresourceDesc : NULL, &pBuffer);
	}
...
CarenResult Shared_D3D11Device::CreateClassLinkage(
	ID3D11Device* Param_MyPointerWork, 
	[Out] ICarenD3D11ClassLinkage^% Param_Out_ClassLinkage)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateClassLinkage(&pLinkage);
...
CarenResult Shared_D3D11Device::CreateComputeShader(
	ID3D11Device* Param_MyPointerWork,
	...
	[Out] ICarenD3D11ComputeShader^% Param_Out_ComputeShader)
{
	...
	if (ObjetoGerenciadoValido(Param_LinkageClass))
	{
		//Obtém um ponteiro para a interface.
		Resultado = Param_LinkageClass->RecuperarPonteiro((LPVOID*)&pLinkage);
	}
	...
	if (Param_ValidarParametros)
	{
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateComputeShader(pSombreadorCompilado, SizeSombreador, ObjetoValido(pLinkage) ? pLinkage : NULL, NULL);
	}
	else
	{
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateComputeShader(pSombreadorCompilado, SizeSombreador, ObjetoValido(pLinkage) ? pLinkage : NULL, &pComputeShader);
	}
...
CarenResult Shared_D3D11Device::CreateCounter(
	ID3D11Device* Param_MyPointerWork,
	Estruturas::CA_D3D11_DESC_CONTADOR^% Param_DescContador,
	[Out] ICarenD3D11Counter^% Param_Out_ContadorGPU)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateCounter(pDescCounter, &pCounter);
...
CarenResult Shared_D3D11Device::CreateDeferredContext(
	ID3D11Device* Param_MyPointerWork,
	...
	[Out] ICarenD3D11DeviceContext^% Param_Out_D3D11Contexto)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateDeferredContext(Param_FlagsContexto, &pContexto);
...
CarenResult Shared_D3D11Device::CreateDepthStencilState(
	ID3D11Device* Param_MyPointerWork,
	...
	[Out] ICarenD3D11DepthStencilState^% Param_Out_InterfaceStencil)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateDepthStencilState(pDescStencil, &pStencilState);
...
CarenResult Shared_D3D11Device::CreateDepthStencilView(
	ID3D11Device* Param_MyPointerWork,
...
)
{
	...
	if (Param_ValidarParametros)
	{
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateDepthStencilView(pRecurso, pDescStencilView ? pDescStencilView : NULL, NULL);
	}
	else
	{
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateDepthStencilView(pRecurso, pDescStencilView ? pDescStencilView : NULL, &pStencilView);
	}
...
CarenResult Shared_D3D11Device::CreateDomainShader(
	ID3D11Device* Param_MyPointerWork,
	...
	[Out] ICarenD3D11DomainShader^% Param_Out_DomainShader)
{
	...
	if (Param_ValidarParametros)
	{
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateDomainShader(pShader, SizeShader, ObjetoValido(pLinkage) ? pLinkage : NULL, NULL);
	}
	else
	{
		//Não está validando os parametros, portanto, a interface de retorno não é NULA.
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateDomainShader(pShader, SizeShader, ObjetoValido(pLinkage) ? pLinkage : NULL, &pDomainShader);
	}
...
CarenResult Shared_D3D11Device::CreateGeometryShader(
	ID3D11Device* Param_MyPointerWork,
	ICaren^ Param_ShaderByteCode,
	UInt64 Param_TamanhoByteCode,
	ICarenD3D11ClassLinkage^ Param_Linkage,
	...
	[Out] ICarenD3D11GeometryShader^% Param_Out_GeometryShader)
{
	...
	if (Param_ValidarParametros)
	{
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateGeometryShader(pShader, SizeShader, ObjetoValido(pLinkage) ? pLinkage : NULL, NULL);
	}
	else
	{
		//Não está validando os parametros, portanto, a interface de retorno não é NULA.
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateGeometryShader(pShader, SizeShader, ObjetoValido(pLinkage) ? pLinkage : NULL, &pInterfaceReturn);
	}
...
/// <param name="Param_ValidarParametros">Defina True para validar os outros parmetros. Param_Out_GeometryShader retorna NULL se esse paramêtro for verdadeiro.</param>
/// <param name="Param_Out_GeometryShader">Recebe a interface do Geometry Shader.</param>
CarenResult Shared_D3D11Device::CreateGeometryShaderWithStreamOutput
(
	ID3D11Device* Param_MyPointerWork,
	ICaren^ Param_ShaderByteCode,
	UInt64 Param_TamanhoByteCode,
...
)
{
	...
	if (Param_ValidarParametros)
	{
		...
		Hr = Param_MyPointerWork->CreateGeometryShaderWithStreamOutput(pShaderByteCode,
			SizeShader,
...
CarenResult Shared_D3D11Device::CreateHullShader(
	ID3D11Device* Param_MyPointerWork,
	ICaren^ Param_ShaderByteCode,
	UInt64 Param_TamanhoByteCode,
	ICarenD3D11ClassLinkage^ Param_Linkage,
...
)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateHullShader(pShader, SizeShader, ObjetoValido(pLinkage) ? pLinkage : NULL, &pHullShader);
...
CarenResult Shared_D3D11Device::CreateInputLayout(
	ID3D11Device* Param_MyPointerWork,
...
)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateInputLayout(pArrayDesc, Param_CountElementos, pShaderByteCode, SizeByteCode, &pInterfaceReturn);
...
CarenResult Shared_D3D11Device::CreatePixelShader(
	ID3D11Device* Param_MyPointerWork,
	ICaren^ Param_ShaderByteCode,
	UInt64 Param_TamanhoByteCode,
	ICarenD3D11ClassLinkage^ Param_Linkage,
	...
	[Out] ICarenD3D11PixelShader^% Param_Out_PixelShader)
{
	...
	if (Param_ValidarParametros)
	{
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreatePixelShader(pShader, SizeShader, ObjetoValido(pLinkage) ? pLinkage : NULL, NULL);
	}
	else
	{
		//Não está validando os parametros, portanto, a interface de retorno não é NULA.
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreatePixelShader(pShader, SizeShader, ObjetoValido(pLinkage) ? pLinkage : NULL, &pPixelShader);
	}
...
CarenResult Shared_D3D11Device::CreatePredicate(
	ID3D11Device* Param_MyPointerWork,
	...
	[Out] ICarenD3D11Predicate^% Param_Out_Predicado)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreatePredicate(pDesc, &pInterfaceReturn);
...
CarenResult Shared_D3D11Device::CreateQuery(
	ID3D11Device* Param_MyPointerWork,
	...
	[Out] ICarenD3D11Query^% Param_Out_Query)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateQuery(pDesc, &pInterfaceReturn);
...
CarenResult Shared_D3D11Device::CreateRasterizerState(
	ID3D11Device* Param_MyPointerWork,
	...
	[Out] ICarenD3D11RasterizerState^% Param_Out_Rasterizador)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateRasterizerState(pDesc, &pInterfaceReturn);
...
CarenResult Shared_D3D11Device::CreateRenderTargetView(
	ID3D11Device* Param_MyPointerWork,
	ICarenD3D11Resource^ Param_Resource,
	...
	[Out] ICarenD3D11RenderTargetView^% Param_Out_ViewRender)
{
	...
	if (Param_ValidarParametros)
	{
		...
		Hr = Param_MyPointerWork->CreateRenderTargetView(pRecurso, pDesc ? pDesc : NULL, NULL);
	}
	else
	{
		//Não está validando os parametros.
		//Sendo assim, deve esperar que uma interface seja retornada.
		Hr = Param_MyPointerWork->CreateRenderTargetView(pRecurso, pDesc ? pDesc : NULL, &pInterfaceReturn);
	}
...
CarenResult Shared_D3D11Device::CreateSamplerState(
	ID3D11Device* Param_MyPointerWork,
	...
	[Out] ICarenD3D11SamplerState^% Param_Out_SamplerState)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateSamplerState(pDesc, &pInterfaceReturn);
...
CarenResult Shared_D3D11Device::CreateShaderResourceView(
	ID3D11Device* Param_MyPointerWork,
	ICarenD3D11Resource^ Param_Recurso,
	...
	[Out] ICarenD3D11ShaderResourceView^% Param_Out_ShaderView)
{
	...
	if (Param_ValidarParametros)
	{
		//Só está validando os parametros.
		//Sendo assm, uma interface não será retornada e deve ser definida como NULL.
		Hr = Param_MyPointerWork->CreateShaderResourceView(pRecurso, pDesc ? pDesc : NULL, NULL);
	}
	else
	{
		//Não está validando os parametros.
		//Sendo assim, deve esperar que uma interface seja retornada.
		Hr = Param_MyPointerWork->CreateShaderResourceView(pRecurso, pDesc ? pDesc : NULL, &pInterfaceReturn);
	}
...
CarenResult Shared_D3D11Device::CreateTexture1D(
	ID3D11Device* Param_MyPointerWork,
	...
	[Out] ICarenD3D11Texture1D^% Param_Out_Textura1D)
{
	...
	if (Param_ValidarParametros)
	{
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateTexture1D(pDesc, pArraySubresourceData ? pArraySubresourceData : NULL, NULL);
	}
	else
	{
		//Não está validando os parametros, portanto, a interface de retorno não é NULA.
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateTexture1D(pDesc, pArraySubresourceData ? pArraySubresourceData : NULL, &pInterfaceReturn);
	}
...
CarenResult Shared_D3D11Device::CreateTexture2D(
	ID3D11Device* Param_MyPointerWork,
	...
	[Out] ICarenD3D11Texture2D^% Param_Out_Textura2D)
{
	...
	if (Param_ValidarParametros)
	{
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateTexture2D(pDesc, pArraySubresourceData ? pArraySubresourceData : NULL, NULL);
	}
	else
	{
		//Não está validando os parametros, portanto, a interface de retorno não é NULA.
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateTexture2D(pDesc, pArraySubresourceData ? pArraySubresourceData : NULL, &pInterfaceReturn);
	}
...
CarenResult Shared_D3D11Device::CreateTexture3D(
	ID3D11Device* Param_MyPointerWork,
	...
	[Out] ICarenD3D11Texture3D^% Param_Out_Textura3D)
{
	...
	if (Param_ValidarParametros)
	{
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateTexture3D(pDesc, pArraySubresourceData ? pArraySubresourceData : NULL, NULL);
	}
	else
	{
		//Não está validando os parametros, portanto, a interface de retorno não é NULA.
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateTexture3D(pDesc, pArraySubresourceData ? pArraySubresourceData : NULL, &pInterfaceReturn);
	}
...
CarenResult Shared_D3D11Device::CreateUnorderedAccessView(
	ID3D11Device* Param_MyPointerWork,
	ICarenD3D11Resource^ Param_Recurso,
	...
	[Out] ICarenD3D11UnorderedAccessView^% Param_Out_Interface)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->CreateUnorderedAccessView(pRecurso, pDesc ? pDesc : NULL, &pInterfaceReturn);
...
CarenResult Shared_D3D11Device::CreateVertexShader(
	ID3D11Device* Param_MyPointerWork,
	ICaren^ Param_ShaderByteCode,
	UInt64 Param_TamanhoByteCode,
	ICarenD3D11ClassLinkage^ Param_Linkage,
	...
	[Out] ICarenD3D11VertexShader^% Param_Out_VertexShader)
{
	...
	if (Param_ValidarParametros)
	{
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateVertexShader(pShader, SizeShader, ObjetoValido(pLinkage) ? pLinkage : NULL, NULL);
	}
	else
	{
		//Não está validando os parametros, portanto, a interface de retorno não é NULA.
		...
		//Chama o método para realizar a operação.
		Hr = Param_MyPointerWork->CreateVertexShader(pShader, SizeShader, ObjetoValido(pLinkage) ? pLinkage : NULL, &pVertexShader);
	}
...
CarenResult Shared_D3D11Device::GetCreationFlags(
	ID3D11Device* Param_MyPointerWork, 
	[Out] Enumeracoes::CA_D3D11_CREATE_DEVICE_FLAG% Param_Out_Flags)
{
	...
	//Chama o método para realizar a operação.
	Param_Out_Flags = static_cast<Enumeracoes::CA_D3D11_CREATE_DEVICE_FLAG>(Param_MyPointerWork->GetCreationFlags());
...
/// </summary>
CarenResult Shared_D3D11Device::GetDeviceRemovedReason(ID3D11Device* Param_MyPointerWork)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetDeviceRemovedReason();
...
CarenResult Shared_D3D11Device::GetExceptionMode(
	ID3D11Device* Param_MyPointerWork, 
	[Out] Enumeracoes::CA_D3D11_RAISE_FLAG% Param_Out_Except)
{
	...
	//Chama o método para realizar a operação.
	Param_Out_Except = static_cast<Enumeracoes::CA_D3D11_RAISE_FLAG>(Param_MyPointerWork->GetExceptionMode());
...
CarenResult Shared_D3D11Device::GetFeatureLevel(
	ID3D11Device* Param_MyPointerWork, 
	[Out] Enumeracoes::CA_D3D_FEATURE_LEVEL% Param_Out_NivelRecurso)
{
	...
	//Chama o método para realizar a operação.
	NivelRecursoHardware = Param_MyPointerWork->GetFeatureLevel();
...
CarenResult Shared_D3D11Device::GetImmediateContext(
	ID3D11Device* Param_MyPointerWork, 
	[Out] ICarenD3D11DeviceContext^% Param_Out_ImediateContextInterface)
{
	...
	//Chama o método para realizar a operação.
	Param_MyPointerWork->GetImmediateContext(&pContexto);
...
CarenResult Shared_D3D11Device::GetPrivateData(
	ID3D11Device* Param_MyPointerWork,
	String^ Param_Guid,
	...
	[Out] ICarenBuffer^% Param_Out_Buffer)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->GetPrivateData(GuidBuffer, &TamanhoBuffer, &pData);
...
CarenResult Shared_D3D11Device::OpenSharedResource(
	ID3D11Device* Param_MyPointerWork,
	IntPtr Param_Handle,
	...
	ICaren^ Param_Out_InterfaceSolicitada)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->OpenSharedResource(pHandle, GuidInterface, &pInterface);
...
CarenResult Shared_D3D11Device::SetExceptionMode(
	ID3D11Device* Param_MyPointerWork, 
	UInt32 Param_RaiseFlags)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->SetExceptionMode(Param_RaiseFlags);
...
CarenResult Shared_D3D11Device::SetPrivateData(
	ID3D11Device* Param_MyPointerWork,
	String^ Param_Guid,
	UInt32 Param_TamanhoBuffer,
	ICarenBuffer^ Param_Buffer)
{
	...
	//Chama o método para realizar a operação.
	Hr = Param_MyPointerWork->SetPrivateData(GuidBuffer, Param_TamanhoBuffer, pData);
...
CarenResult Shared_D3D11Device::SetPrivateDataInterface(
	ID3D11Device* Param_MyPointerWork,
	String^ Param_Guid,
