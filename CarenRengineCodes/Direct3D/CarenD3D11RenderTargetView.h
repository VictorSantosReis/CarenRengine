...
/// <summary>
/// (Concluido - Fase de Testes) - Classe responsável por representar um (Alvo de visualização do render) que identifica os subrecursos de renderização alvo que podem ser acessados durante a renderização.
/// </summary>
public ref class CarenD3D11RenderTargetView :public ICarenD3D11RenderTargetView
{
	/////////////////////////////////////////
...
public:
	static operator CarenD3D11RenderTargetView^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD3D11RenderTargetView^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD3D11RenderTargetView();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID3D11RenderTargetView*>(Param_Pointer.ToPointer());
...
public:
	...
	/// </summary>
	/// <param name="Param_Out_CarenD3D11RenderTargetView">Recebe um ponteiro para a interface (Vazia).</param>
	static CarenResult CriarInstanciaVazia([Out] ICarenD3D11RenderTargetView^% Param_Out_CarenD3D11RenderTargetView)
	{
		//Variavel a ser retornada.
		CarenResult Resultado = CarenResult(E_FAIL, false);

		//Cria a interface
		Param_Out_CarenD3D11RenderTargetView = gcnew CarenD3D11RenderTargetView();
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD3D11RenderTargetView::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
...
public:
	...
	virtual CarenResult GetDesc([Out] Estruturas::CA_D3D11_RENDER_TARGET_VIEW_DESC^% Param_Out_DescRenderDestino);
