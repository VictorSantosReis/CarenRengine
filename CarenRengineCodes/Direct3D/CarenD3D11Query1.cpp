...
//Destruidor.
CarenD3D11Query1::~CarenD3D11Query1()
{
...
CarenResult CarenD3D11Query1::ConsultarInterface(String^ Param_Guid, ICaren^ Param_InterfaceSolicitada)
{
	...
	Hr = (reinterpret_cast<ID3D11Query1*>(PonteiroTrabalho))->QueryInterface(GuidInterface, (LPVOID*)&pInterfaceSolcitada);
...
CarenResult CarenD3D11Query1::AdicionarPonteiro(IntPtr Param_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	PonteiroTrabalho = reinterpret_cast<ID3D11Query1*>(Param_PonteiroNativo.ToPointer());
...
CarenResult CarenD3D11Query1::AdicionarPonteiro(LPVOID Param_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Converte o ponteiro para o tipo especifico da classe.
	PonteiroTrabalho = reinterpret_cast<ID3D11Query1*>(Param_PonteiroNativo);
...
CarenResult CarenD3D11Query1::RecuperarPonteiro([Out] IntPtr% Param_Out_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
CarenResult CarenD3D11Query1::RecuperarPonteiro(LPVOID* Param_Out_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
CarenResult CarenD3D11Query1::RecuperarReferencias([Out] UInt64% Param_Out_Referencias)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
/// </summary>
CarenResult CarenD3D11Query1::StatusPonteiro()
{
...
/// </summary>
Int32 CarenD3D11Query1::ObterCodigoErro()
{
...
/// </summary>
void CarenD3D11Query1::AdicionarReferencia()
{
	//Adiciona uma referência ao ponteiro
	(reinterpret_cast<ID3D11Query1*>(PonteiroTrabalho))->AddRef();
}

/// <summary>
/// (Release) - 'Decrementa' a contagem de referência do objeto COM atual.
/// </summary>
void CarenD3D11Query1::LiberarReferencia()
{
	//Libera a referência e obtém a quantidade atual.
	ULONG RefCount = (reinterpret_cast<ID3D11Query1*>(PonteiroTrabalho))->Release();
...
/// </summary>
void CarenD3D11Query1::LimparDados()
{
...
/// </summary>
void CarenD3D11Query1::Finalizar()
{
	//////////////////////
	//Código de descarte//
	//////////////////////
	...
	this->~CarenD3D11Query1();
}



//Métodos da interface proprietária(ICarenD3D11Query1)
...
CarenResult CarenD3D11Query1::GetDesc1([Out] Estruturas::CA_D3D11_QUERY_DESC1^% Param_Out_DescConsulta)
{
	...
	PonteiroTrabalho->GetDesc1(&QueryDesc);

	//Converte a estrutura nativa para a gerenciada e define no parametro de saida.
	Param_Out_DescConsulta = Util.ConverterD3D11_QUERY_DESC1UnManaged_ToManaged(&QueryDesc);
...
//Métodos da interface ICarenD3D11Query

/// <summary>
/// (GetDesc) - Obtém uma descrição de consulta.
/// </summary>
/// <param name="Param_Out_DescConsulta">Retorna uma estrutura para a descrição da consulta.</param>
CarenResult CarenD3D11Query1::GetDesc([Out] Estruturas::CA_D3D11_QUERY_DESC^% Param_Out_DescConsulta)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Chama o método para realizar a operação.
	PonteiroTrabalho->GetDesc(&QueryDesc);

	//Converte a estrutura nativa para a gerenciada e define no parametro de saida.
	Param_Out_DescConsulta = Util.ConverterD3D11_QUERY_DESCUnManaged_ToManaged(&QueryDesc);
...
CarenResult CarenD3D11Query1::ObterTamanhoDados([Out] UInt32% Param_TamanhoDados)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);

	//Chama o método para realizar a função
	Param_TamanhoDados = PonteiroTrabalho->GetDataSize();
...
CarenResult CarenD3D11Query1::ObterDispositivo(ICaren^ Param_Out_DispositivoD3D11)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
CarenResult CarenD3D11Query1::ObterDadosPrivados(
	String^ Param_Guid,
...
CarenResult CarenD3D11Query1::DefinirDadosPrivados(
	String^ Param_Guid,
...
CarenResult CarenD3D11Query1::DefinirInterfaceDadosPrivados(
