...
public:
	static operator CarenMFMediaSourceEx^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenMFMediaSourceEx^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenMFMediaSourceEx();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<IMFMediaSourceEx*>(Param_Pointer.ToPointer());
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenMFMediaSourceEx::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
...
public:
	...
	virtual CarenResult GetSourceAttributes([Out] ICarenMFAttributes^% Param_Out_AtributosFonteMidia);
	...
	virtual CarenResult GetStreamAttributes(UInt32 Param_IdentificadorFluxo, [Out] ICarenMFAttributes^% Param_Out_AtributosFluxoFonte);
...
public:
	...
	virtual CarenResult Start(ICarenMFPresentationDescriptor^ Param_DescritorApresentação, String^ Param_GuidTimeFormato, CA_PROPVARIANT^ Param_PosiçãoInicio);
