...
CarenResult CarenMFMediaEventGenerator::AdicionarPonteiro(IntPtr Param_PonteiroNativo)
{
	...
	PonteiroTrabalho = reinterpret_cast<IMFMediaEventGenerator*>(Param_PonteiroNativo.ToPointer());
...
CarenResult CarenMFMediaEventGenerator::AdicionarPonteiro(LPVOID Param_PonteiroNativo)
{
	...
	//Converte o ponteiro para o tipo especifico da classe.
	PonteiroTrabalho = reinterpret_cast<IMFMediaEventGenerator*>(Param_PonteiroNativo);
...
/// </summary>
void CarenMFMediaEventGenerator::LimparDados()
{
...
CarenResult CarenMFMediaEventGenerator::ObterEvento(CA_FLAGS_OBTER_EVENTO Param_Flags, [Out] ICarenMFMediaEvent^% Param_Out_MidiaEvent)
{
...
CarenResult CarenMFMediaEventGenerator::SolicitarProximoEvento(ICarenMFAsyncCallback^ Param_Callback, ICaren^ Param_ObjetoDesconhecido)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
/// <param name="Param_Out_MidiaEvent">Recebe a interface que contém as informações da operação assincrona para o evento notificado. O chamador deve liberar a interface.</param>
CarenResult CarenMFMediaEventGenerator::ConcluirSolicitaçãoEvento(ICarenMFAsyncResult^ Param_ResultAsync, [Out] ICarenMFMediaEvent^% Param_Out_MidiaEvent)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
CarenResult CarenMFMediaEventGenerator::InserirEventoFila(Enumeracoes::CA_TIPO_MIDIA_EVENT Param_TipoEvento, String^ Param_GuidExtendedType, Int32 Param_HResultCode, Estruturas::CA_PropVariant^ Param_Dados) {
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
