...
#include "../pch.h"
#include "CarenD2D1SourceTransform.h"
...
CarenResult CarenD2D1SourceTransform::ConsultarInterface(String^ Param_Guid, ICaren^ Param_InterfaceSolicitada)
{
...
CarenResult CarenD2D1SourceTransform::AdicionarPonteiro(IntPtr Param_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
	...
	PonteiroTrabalho = reinterpret_cast<ID2D1SourceTransform*>(Param_PonteiroNativo.ToPointer());
...
CarenResult CarenD2D1SourceTransform::AdicionarPonteiro(LPVOID Param_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
	...
	//Converte o ponteiro para o tipo especifico da classe.
	PonteiroTrabalho = reinterpret_cast<ID2D1SourceTransform*>(Param_PonteiroNativo);
...
CarenResult CarenD2D1SourceTransform::RecuperarPonteiro([Out] IntPtr% Param_Out_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
CarenResult CarenD2D1SourceTransform::RecuperarPonteiro(LPVOID* Param_Out_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
CarenResult CarenD2D1SourceTransform::RecuperarReferencias([Out] UInt64% Param_Out_Referencias)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
...
/// </summary>
CarenResult CarenD2D1SourceTransform::StatusPonteiro()
{
...
/// </summary>
Int32 CarenD2D1SourceTransform::ObterCodigoErro()
{
...
/// </summary>
void CarenD2D1SourceTransform::AdicionarReferencia()
{
	//Adiciona uma referência ao ponteiro
...
/// </summary>
void CarenD2D1SourceTransform::LiberarReferencia()
{
...
/// </summary>
void CarenD2D1SourceTransform::LimparDados()
{
...
/// </summary>
void CarenD2D1SourceTransform::Finalizar()
{
	//////////////////////
	//Código de descarte//
	//////////////////////
	...
	this->~CarenD2D1SourceTransform();
}



// Métodos da interface proprietária(ICarenD2D1SourceTransform)
...
CarenResult CarenD2D1SourceTransform::Draw(
...
CA_D2D1_POINT_2U^ Param_TargetOrigin)
{
	...
	//Variaveis a serem utilizadas.
	Utilidades Util;
	ID2D1Bitmap1* pBitmapTarget = NULL;
	D2D1_RECT_L* pDrawRect = NULL;
	D2D1_POINT_2U* pTargetOriginPoint = NULL;

	//Recupera o ponteiro para a interface
	Resultado = RecuperarPonteiroCaren(Param_Target, &pBitmapTarget);

	//Sai do método em caso de erro
	SairOnError(Resultado);

	//Converte as estruturas.
	pDrawRect = Util.ConverterD2D1_RECT_LManagedToUnmanaged(Param_DrawRect);
	pTargetOriginPoint = Util.ConverterD2D1_POINT_2UManagedToUnmanaged(Param_TargetOrigin);

	//Chama o método para realizar a operação.
	Hr = PonteiroTrabalho->Draw(pBitmapTarget, pDrawRect, *pTargetOriginPoint);
...
Done:;
	...
	DeletarEstruturaSafe(&pDrawRect);
	DeletarEstruturaSafe(&pTargetOriginPoint);
...
CarenResult CarenD2D1SourceTransform::SetRenderInfo(ICarenD2D1RenderInfo^ Param_RenderInfo)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(ResultCode::ER_FAIL, false);
	...
	//Variaveis a serem utilizadas.
	ID2D1RenderInfo* pRenderInfo = NULL;

	//Recupera o ponteiro para a interface
	Resultado = RecuperarPonteiroCaren(Param_RenderInfo, &pRenderInfo);

	//Sai do método em caso de erro
	SairOnError(Resultado);

	//Chama o método para realizar a operação.
	Hr = PonteiroTrabalho->SetRenderInfo(pRenderInfo);
...
CarenResult CarenD2D1SourceTransform::MapInputRectsToOutputRect(
...
CarenResult CarenD2D1SourceTransform::MapInvalidRect(
...
CarenResult CarenD2D1SourceTransform::MapOutputRectToInputRects(
...
void CarenD2D1SourceTransform::GetInputCount([Out] UInt32% Param_Out_Quantidade)
{
	//Chama o método para realizar a operação.
