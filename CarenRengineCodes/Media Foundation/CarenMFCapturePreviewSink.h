...
public:
	static operator CarenMFCapturePreviewSink^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenMFCapturePreviewSink^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenMFCapturePreviewSink();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<IMFCapturePreviewSink*>(Param_Pointer.ToPointer());
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenMFCapturePreviewSink::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
