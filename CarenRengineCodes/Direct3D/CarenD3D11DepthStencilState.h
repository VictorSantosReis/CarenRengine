...
/// <summary>
/// (Concluido - Fase de Testes) - Classe responsável por conter uma descrição para o (Estado de Profundidade Stêncil) que você pode ligar para a fase de fusão de produção.
/// </summary>
public ref class CarenD3D11DepthStencilState :public ICarenD3D11DepthStencilState
{
	/////////////////////////////////////////
...
public:
	static operator CarenD3D11DepthStencilState^ (IntPtr Param_Pointer)
	{
		//Variavel a ser retornada.
		CarenD3D11DepthStencilState^ ClassResultado = nullptr;
		...
		ClassResultado = gcnew CarenD3D11DepthStencilState();

		//Define o ponteiro na classe.
		ClassResultado->PonteiroTrabalho = reinterpret_cast<ID3D11DepthStencilState*>(Param_Pointer.ToPointer());
...
public:
	...
	/// </summary>
	/// <param name="Param_Out_CarenD3D11DepthStencilState">Recebe um ponteiro para a interface (Vazia).</param>
	static CarenResult CriarInstanciaVazia([Out] ICarenD3D11DepthStencilState^% Param_Out_CarenD3D11DepthStencilState)
	{
		//Variavel a ser retornada.
		CarenResult Resultado = CarenResult(E_FAIL, false);

		//Cria a interface
		Param_Out_CarenD3D11DepthStencilState = gcnew CarenD3D11DepthStencilState();
...
public:
	...
	virtual TypeInterface As(cli::array<Object^>^ Param_Args)
	{
		...
		catch (const std::exception&)
		{
			...
			System::Console::WriteLine(String::Concat(ICarenD3D11DepthStencilState::typeid->Name, " - O método (As<>) falhou ao tentar criar uma instância para o tipo de destino solicitado => ", TypeInterface::typeid->Name));
...
public:
	...
	virtual CarenResult GetDesc([Out] Estruturas::CA_D3D11_DEPTH_STENCIL_DESC^% Param_Out_DescEstadoStencil);
