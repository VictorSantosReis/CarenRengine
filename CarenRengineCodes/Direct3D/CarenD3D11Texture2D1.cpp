...
//Destruidor.
CarenD3D11Texture2D1::~CarenD3D11Texture2D1()
{
...
CarenResult CarenD3D11Texture2D1::ConsultarInterface(String^ Param_Guid, ICaren^ Param_InterfaceSolicitada)
{
	...
	Hr = (reinterpret_cast<ID3D11Texture2D1*>(PonteiroTrabalho))->QueryInterface(GuidInterface, (LPVOID*)&pInterfaceSolcitada);
...
CarenResult CarenD3D11Texture2D1::AdicionarPonteiro(IntPtr Param_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	PonteiroTrabalho = reinterpret_cast<ID3D11Texture2D1*>(Param_PonteiroNativo.ToPointer());
...
CarenResult CarenD3D11Texture2D1::AdicionarPonteiro(LPVOID Param_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Converte o ponteiro para o tipo especifico da classe.
	PonteiroTrabalho = reinterpret_cast<ID3D11Texture2D1*>(Param_PonteiroNativo);
...
CarenResult CarenD3D11Texture2D1::RecuperarPonteiro([Out] IntPtr% Param_Out_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
CarenResult CarenD3D11Texture2D1::RecuperarPonteiro(LPVOID* Param_Out_PonteiroNativo)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
CarenResult CarenD3D11Texture2D1::RecuperarReferencias([Out] UInt64% Param_Out_Referencias)
{
	//Variavel que vai retornar o resultado.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
/// </summary>
CarenResult CarenD3D11Texture2D1::StatusPonteiro()
{
...
/// </summary>
Int32 CarenD3D11Texture2D1::ObterCodigoErro()
{
...
/// </summary>
void CarenD3D11Texture2D1::AdicionarReferencia()
{
	//Adiciona uma referência ao ponteiro
	(reinterpret_cast<ID3D11Texture2D1*>(PonteiroTrabalho))->AddRef();
}

/// <summary>
/// (Release) - 'Decrementa' a contagem de referência do objeto COM atual.
/// </summary>
void CarenD3D11Texture2D1::LiberarReferencia()
{
	//Libera a referência e obtém a quantidade atual.
	ULONG RefCount = (reinterpret_cast<ID3D11Texture2D1*>(PonteiroTrabalho))->Release();
...
/// </summary>
void CarenD3D11Texture2D1::LimparDados()
{
...
/// </summary>
void CarenD3D11Texture2D1::Finalizar()
{
	//////////////////////
	//Código de descarte//
	//////////////////////
	...
	this->~CarenD3D11Texture2D1();
}



//Métodos da interface proprietária(ICarenD3D11Texture2D1)
...
CarenResult CarenD3D11Texture2D1::GetDesc1([Out] Estruturas::CA_D3D11_TEXTURE2D_DESC1^% Param_Out_PropsTextura2D)
{
	...
	//Variaveis
	Utilidades Util;
	D3D11_TEXTURE2D_DESC1 DescTextura2D;

	//Chama o método para realizar a operação
	PonteiroTrabalho->GetDesc1(&DescTextura2D);

	//Converte a estrutura e define no parametro de saida.
	Param_Out_PropsTextura2D = Util.ConverterD3D11_TEXTURE2D_DESC1UnManaged_ToManaged(&DescTextura2D);
...
//Métodos da interface ICarenD3D11Texture2D

/// <summary>
/// (GetDesc) - Obtem as propriedades do recurso de textura.
/// </summary>
/// <param name="Param_Out_PropsTextura2D">Retorna uma estrutura com as descrições do recuso.</param>
CarenResult CarenD3D11Texture2D1::GetDesc([Out] Estruturas::CA_D3D11_TEXTURE2D_DESC^% Param_Out_PropsTextura2D)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
	...
	//Variaveis
	Utilidades Util;
	D3D11_TEXTURE2D_DESC DescTextura2D;

	//Chama o método para realizar a operação
	PonteiroTrabalho->GetDesc(&DescTextura2D);

	//Converte a estrutura e define no parametro de saida.
	Param_Out_PropsTextura2D = Util.ConverterD3D11_TEXTURE2D_DESCUnManaged_ToManaged(&DescTextura2D);
...
CarenResult CarenD3D11Texture2D1::ObterPrioridadeDespejo([Out] Enumeracoes::CA_DXGI_PRIORIDADE_RECURSO% Param_Out_Prioridade)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
CarenResult CarenD3D11Texture2D1::ObterTipo([Out] Enumeracoes::CA_D3D11_TIPO_RECURSO% Param_Out_TipoRecurso)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
CarenResult CarenD3D11Texture2D1::DefinirPrioridadeDespejo(Enumeracoes::CA_DXGI_PRIORIDADE_RECURSO Param_PrioridadeRecurso)
{
...
CarenResult CarenD3D11Texture2D1::ObterDispositivo(ICaren^ Param_Out_DispositivoD3D11)
{
	//Variavel a ser retornada.
	CarenResult Resultado = CarenResult(E_FAIL, false);
...
CarenResult CarenD3D11Texture2D1::ObterDadosPrivados(
	String^ Param_Guid,
...
CarenResult CarenD3D11Texture2D1::DefinirDadosPrivados(
	String^ Param_Guid,
...
CarenResult CarenD3D11Texture2D1::DefinirInterfaceDadosPrivados(
